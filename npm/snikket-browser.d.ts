...
export namespace snikket {
	export class Chat {
		protected constructor(client: snikket.Client, stream: snikket.GenericStream, persistence: snikket.Persistence, chatId: string, uiState?: enums.UiState, extensions?: snikket.Stanza, readUpToId?: string, readUpToBy?: string);
		/**
		 * ID of this Chat
		 */
		 ...
		 */
		addReaction(m: snikket.ChatMessage, reaction: string): void;
		/**
		 ...
		 */
		removeReaction(m: snikket.ChatMessage, reaction: string): void;
		/**
...
export namespace snikket {
	export class Channel extends snikket.Chat {
		protected constructor(client: snikket.Client, stream: snikket.GenericStream, persistence: snikket.Persistence, chatId: string, uiState?: enums.UiState, extensions?: snikket.Stanza, readUpToId?: string, readUpToBy?: string, disco?: snikket.Caps);
...
export namespace snikket {
	export class SerializedChat {
		constructor(chatId: string, trusted: boolean, avatarSha1: ArrayBuffer, presence: Map<string,snikket.Presence>, displayName: string, uiState: enums.UiState, extensions: string, readUpToId: string, readUpToBy: string, disco: snikket.Caps, klass: string);
		chatId: string;
		trusted: boolean;
...
export namespace snikket {
	export class Client extends snikket.EventEmitter {
		 ...
		 */
		addChatMessageListener(handler: (arg0: snikket.ChatMessage) => void): void;
		/**
...
export namespace snikket {
	export class Notification {
		constructor(title: string, body: string, accountId: string, chatId: string, messageId: string, imageUri: string, lang: string, timestamp: string);
...
export namespace snikket {
	export class Participant {
		protected constructor(displayName: string, photoUri: string, placeholderUri: string);
		displayName: string;
		photoUri: string;
...
export namespace snikket {
	export class ReactionUpdate {
		constructor(updateId: string, serverId: string, serverIdBy: string, localId: string, chatId: string, timestamp: string, senderId: string, reactions: string[], append?: boolean);
		updateId: string;
		serverId: string;
		serverIdBy: string;
		localId: string;
		...
		reactions: string[];
		append: boolean;
		getReactions(existingReactions: string[]): string[];
	}
}
...
export namespace snikket {
	export interface Persistence {
		...
		storeChat(accountId: string, chat: snikket.Chat): void;
		...
		storeMessage(accountId: string, message: snikket.ChatMessage, callback: (arg0: snikket.ChatMessage) => void): void;
