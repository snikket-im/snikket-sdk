...
class borogove_Caps {
	constructor(node,identities,features,data,ver) {
		...
		if(ver == null) {
			...
			data.sort(function(x,y) {
				...
				let tmp2 = borogove_DataForm.field(y,"FORM_TYPE");
				return Reflect.compare(tmp1,tmp2 != null ? borogove_Field.get_value(tmp2) : null);
			});
		}
		this.node = node;
	...
	hashInput() {
		...
		while(_g2 < _g3.length) _g3[_g2++].writeTo(s);
		s.writeByte(28);
		let _g4 = 0;
		let _g5 = this.data;
		...
		while(_g4 < _g5.length) {
			...
			while(_g < fields.length) {
				...
				if((tmp != null ? tmp : "") != "FORM_TYPE") {
					...
					values.sort(Reflect.compare);
					let _g = 0;
					while(_g < values.length) s += values[_g++] + "<";
				}
			}
		}
...
class borogove_DirectChat extends borogove_Chat {
	...
	sendMessage(message) {
		...
		case 3:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "borogove/Chat.hx", lineNumber : 1075, className : "borogove.DirectChat", methodName : "sendMessage", customParams : [fromStanza]});
	...
	sendMessageStanza(stanza,outboxItem) {
		...
		while(_g < _g1) {
			...
			if(counters.length > 1 && stanza.getChild("addresses","http://jabber.org/protocol/address") == null) {
				...
				let _g = 0;
				while(_g < counters.length) addresses.tag("address",{ type : "to", jid : counters[_g++], delivered : "true"}).up();
				addresses.up();
			}
		...
		thenshim_Promise.then(thenshim_PromiseTools.all(result),function(stanzas) {
			outboxItem.handle(function() {
				let _g = 0;
				while(_g < stanzas.length) _gthis.client.sendStanza(stanzas[_g++]);
			});
		});
	}
	lastMessageId() {
...
class borogove_Channel extends borogove_Chat {
		...
		(refresh ? $bind(this,this.refreshDisco) : function(cb) {
			...
			_gthis.stream.sendIq(new borogove_Stanza("iq",{ type : "get", to : _gthis.getFullJid().asString()}).tag("ping",{ xmlns : "urn:xmpp:ping"}).up(),function(response) {
				if(response.attr["type"] == "error") {
					...
					haxe_Log.trace("SYNC: self-ping fail, join",{ fileName : "borogove/Chat.hx", lineNumber : 1286, className : "borogove.Channel", methodName : "selfPing", customParams : [_gthis.chatId]});
					_gthis.join();
				} else {
					_gthis.selfPingSuccess();
				}
	...
	doSync(lastId) {
		...
		this.sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 1:
					...
					while(_g4 < _g5.length) _gthis.client.fetchMediaByHash([_g5[_g4++]],[_g3.from]);
		...
		this.sync.onError(function(stanza) {
			...
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC failed",{ fileName : "borogove/Chat.hx", lineNumber : 1521, className : "borogove.Channel", methodName : "doSync", customParams : [_gthis.chatId,stanza]});
			}
		});
		this.sync.fetchNext();
	}
	...
	sendMessage(message) {
		...
		case 3:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "borogove/Chat.hx", lineNumber : 1733, className : "borogove.Channel", methodName : "sendMessage", customParams : [fromStanza]});
			throw haxe_Exception.thrown("Trying to send invalid message.");
		}
	}
	removeReaction(m,reaction) {
...
class borogove_Client extends borogove_EventEmitter {
	...
	_hx_constructor(accountId,persistence) {
		...
		this.stream.onIq(borogove_IqRequestType.Set,"jingle","urn:xmpp:jingle:1",function(stanza) {
			...
			if(jingle.attr["action"] == "session-initiate") {
				if(session != null) {
					try {
						...
						haxe_Log.trace("Bad session-inititate",{ fileName : "borogove/Client.hx", lineNumber : 219, className : "borogove.Client", methodName : "new", customParams : [_g1]});
			...
			if(session != null && jingle.attr["action"] == "session-accept") {
				try {
					...
					let _g1 = haxe_Exception.caught(_g);
					haxe_Log.trace("session-accept failed",{ fileName : "borogove/Client.hx", lineNumber : 234, className : "borogove.Client", methodName : "new", customParams : [_g1]});
				}
			}
		...
		this.stream.on("presence",function(event) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == null) {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "borogove/Client.hx", lineNumber : 336, className : "borogove.Client", methodName : "new"});
					return borogove_EventResult.EventUnhandled;
				}
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == "unavailable") {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "borogove/Client.hx", lineNumber : 408, className : "borogove.Client", methodName : "new"});
					return borogove_EventResult.EventUnhandled;
				}
	...
	processLiveMessage(stanza,fwd,encryptionInfo) {
		...
		case 4:
			...
			break;
		default:
			haxe_Log.trace("Ignoring non-chat message: " + stanza.toString(),{ fileName : "borogove/Client.hx", lineNumber : 491, className : "borogove.Client", methodName : "processLiveMessage"});
		}
		...
		if(jmiPro != null && jmiPro.attr["id"] != null) {
			...
			if(session != null) {
				try {
					...
					let _g1 = haxe_Exception.caught(_g);
					haxe_Log.trace("JMI proceed failed",{ fileName : "borogove/Client.hx", lineNumber : 535, className : "borogove.Client", methodName : "processLiveMessage", customParams : [_g1]});
				}
			}
		}
		...
		haxe_Log.trace("pubsubEvent " + Std.string(pubsubEvent != null),{ fileName : "borogove/Client.hx", lineNumber : 606, className : "borogove.Client", methodName : "processLiveMessage"});
		if(pubsubEvent != null && pubsubEvent.getFrom() != null) {
			...
			haxe_Log.trace("pubsubNode == " + pubsubNode,{ fileName : "borogove/Client.hx", lineNumber : 633, className : "borogove.Client", methodName : "processLiveMessage"});
		}
	}
	start() {
	...
	onConnected(data) {
		...
		this.rosterGet();
		haxe_Log.trace("SYNC: bookmarks",{ fileName : "borogove/Client.hx", lineNumber : 825, className : "borogove.Client", methodName : "onConnected"});
		this.bookmarksGet(function() {
			haxe_Log.trace("SYNC: MAM",{ fileName : "borogove/Client.hx", lineNumber : 827, className : "borogove.Client", methodName : "onConnected"});
			_gthis.sync(function(syncFinished) {
				if(!syncFinished) {
					haxe_Log.trace("SYNC: failed",{ fileName : "borogove/Client.hx", lineNumber : 830, className : "borogove.Client", methodName : "onConnected"});
					_gthis.inSync = false;
					_gthis.stream.disconnect();
					return;
				}
				haxe_Log.trace("SYNC: details",{ fileName : "borogove/Client.hx", lineNumber : 837, className : "borogove.Client", methodName : "onConnected"});
				_gthis.inSync = true;
				thenshim_Promise.then(_gthis.persistence.getChatsUnreadDetails(_gthis.accountId(),_gthis.chats),function(details) {
					...
					haxe_Log.trace("SYNC: done",{ fileName : "borogove/Client.hx", lineNumber : 863, className : "borogove.Client", methodName : "onConnected"});
				});
			});
		});
			...
			return thenshim_Promise._new(function(resolve,reject) {
				tink_io_Source.chunked(new tink_io_js_BlobSource(source.name,source,0,16777216)).forEach(tink_streams_Handler.ofSafeSync(function(chunk) {
					...
					if(o._hx_index == 3) {
						...
						haxe_Log.trace("Error computing attachment hash",{ fileName : "borogove/Client.hx", lineNumber : 890, className : "borogove.Client", methodName : "prepareAttachment", customParams : [o]});
						reject(o);
					}
				});
			});
	...
	prepareAttachmentFor(source,services,hashes,callback) {
		if(services.length < 1) {
			haxe_Log.trace("No HTTP upload service found",{ fileName : "borogove/Client.hx", lineNumber : 899, className : "borogove.Client", methodName : "prepareAttachmentFor"});
			callback(null);
			return;
		}
		...
		httpUploadSlot.onFinished(function() {
			...
			if(slot == null) {
				...
				tink_http_FetchResponse.all(tink_http_Fetch.fetch(tink_Url.fromString(slot.put),{ method : "PUT", headers : slot.putHeaders.concat([new tink_http_HeaderField("Content-Length".toLowerCase(),tink_http_HeaderValue.ofInt(source.size))]), body : tink_io_RealSourceTools.idealize(new tink_io_js_BlobSource(source.name,source,0,16777216),function(e) {
					haxe_Log.trace("WUT",{ fileName : "borogove/Client.hx", lineNumber : 909, className : "borogove.Client", methodName : "prepareAttachmentFor", customParams : [e]});
		...
		window.crypto.subtle.exportKey("raw",vapid_key.publicKey).then(function(vapid_public_raw) {
			return reg.pushManager.subscribe({ userVisibleOnly : true, applicationServerKey : vapid_public_raw}).then(function(pushSubscription) {
				if(pushSubscription == null) {
					haxe_Log.trace("WebPush subscription failed",{ fileName : "borogove/Client.hx", lineNumber : 1129, className : "borogove.Client", methodName : "subscribePush"});
					return;
				}
	...
	doSync(callback,lastId) {
		...
		sync.onMessages(function(messageList) {
			...
			haxe_Log.trace("SYNC: MAM page wait for writes",{ fileName : "borogove/Client.hx", lineNumber : 1827, className : "borogove.Client", methodName : "doSync"});
			thenshim_Promise.then(thenshim_PromiseTools.all(promises),function(results) {
				...
				haxe_Log.trace("SYNC: error",{ fileName : "borogove/Client.hx", lineNumber : 1854, className : "borogove.Client", methodName : "doSync", customParams : [e == null ? "null" : Std.string(e)]});
				callback(false);
			});
		});
		sync.onError(function(stanza) {
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC: error",{ fileName : "borogove/Client.hx", lineNumber : 1863, className : "borogove.Client", methodName : "doSync", customParams : [stanza]});
				if(callback != null) {
					callback(false);
				}
			}
...
class borogove_CommandSession {
	...
	execute(action,data,formIdx) {
		...
		if(data != null || extendedAction) {
			toSubmit = this.forms[formIdx != null ? formIdx : 0].submit(data);
...
class borogove_Field {
	...
	static set_value(this1,val) {
		...
		let _g = 0;
		while(_g < val.length) this1.textTag("value",val[_g++]);
		return val;
	}
	static get_type(this1) {
...
class borogove_FormSubmitBuilder {
	...
	submit(form) {
		...
		if(form != null) {
			...
			while(_g < _g1.length) {
				...
				if(this.data.get(tmp != null ? tmp : "") == null && borogove_Field.get_value(f).length > 0) {
					...
					while(_g < _g1.length) tag.textTag("value",_g1[_g++]);
					tag.up();
				} else {
					let tmp;
					...
					if(tmp) {
						let tmp = f.attr["var"];
						haxe_Log.trace("No value provided for required field",{ fileName : "borogove/Form.hx", lineNumber : 72, className : "borogove.FormSubmitBuilder", methodName : "submit", customParams : [tmp != null ? tmp : ""]});
						return null;
					}
				}
			}
		...
		while(!_g_lastStep.done) {
			...
			let _g = 0;
			while(_g < _g_value.length) tag.textTag("value",_g_value[_g++]);
			tag.up();
		}
		return toSubmit;
	}
		...
		while(_g < _g1.length) {
			...
			if(!hasLayout && child.name == "field" && (child.attr["xmlns"] == null || child.attr["xmlns"] == "jabber:x:data")) {
				...
				if((attr == "jid-single" ? "text-single" : attr == "jid-multi" ? "text-multi" : attr) == "fixed" && fld.attr["label"] == null) {
					...
					while(_g < _g1.length) items.push(new borogove_FormItem(_g1[_g++],null,null));
				} else {
					let attr = fld.attr["type"];
		...
		if(((data) instanceof Map)) {
			...
			while(!_g_lastStep.done) {
				...
				if(typeof(_g_value) == "string") {
					...
					let _g = 0;
					let _g1 = _g_value;
					while(_g < _g1.length) builder.add(_g_key,_g1[_g++]);
				}
			}
			...
			while(_g_index < _g_keys.length) {
				...
				if(typeof(_g_value) == "string") {
					...
					let _g = 0;
					let _g1 = _g_value;
					while(_g < _g1.length) builder.add(key,_g1[_g++]);
				}
			}
		}
...
class borogove_Message {
	...
	static fromStanza(stanza,localJid,addContext,encryptionInfo) {
		while(true) {
			...
			if(addresses != null) {
				...
				while(_g < _g1.length) {
					...
					if(address.attr["type"] == "noreply") {
						...
						haxe_Log.trace("No support for addressing to non-jid",{ fileName : "borogove/Message.hx", lineNumber : 159, className : "borogove.Message", methodName : "fromStanza", customParams : [address]});
			...
			if(tmp1) {
				haxe_Log.trace("Don't know what chat message without from in replyTo belongs in",{ fileName : "borogove/Message.hx", lineNumber : 187, className : "borogove.Message", methodName : "fromStanza", customParams : [stanza]});
				return new borogove_Message(msg.chatId(),msg.get_senderId(),msg.threadId,borogove_MessageStanza.UnknownMessageStanza(stanza),encryptionInfo);
			}
			if(addContext != null) {
	...
	fetchNext() {
		...
		let eventToken = this.stream.on("message",function(event) {
			...
			if(originalMessage.hasChild("encrypted",borogove_NS.OMEMO)) {
				...
				haxe_Log.trace("MAM: Processing non-OMEMO message from " + originalMessage.attr["from"],{ fileName : "borogove/MessageSync.hx", lineNumber : 143, className : "borogove.MessageSync", methodName : "fetchNext"});
		...
		query.onFinished(function() {
			...
			if(result == null) {
				haxe_Log.trace("Error from MAM, stopping sync",{ fileName : "borogove/MessageSync.hx", lineNumber : 162, className : "borogove.MessageSync", methodName : "fetchNext"});
	...
	return thenshim_Promise._new(function(resolve,reject) {
		tink_http_FetchResponse.all(tink_http_Fetch.fetch(tink_Url.fromString(url),{ method : "HEAD"})).handle(function(o) {
			...
			case 0:
				...
				while(_g < _g1.length) if(regex.match(_g1[_g++])) {
...
class borogove_XEP0393 {
	...
	static render(xhtml,inPre) {
		...
		while(_g < _g1.length) s_b += Std.string(borogove_XEP0393.renderNode(_g1[_g++],xhtml.name == "pre"));
		if(xhtml.name == "b" || xhtml.name == "strong") {
			s_b += "*";
		}
		if(xhtml.name == "i" || xhtml.name == "em") {
...
class borogove_calls_IncomingProposedSession {
	...
	terminate() {
		haxe_Log.trace("Tried to terminate before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 117, className : "borogove.calls.IncomingProposedSession", methodName : "terminate", customParams : [this]});
	}
	contentAdd(_) {
		haxe_Log.trace("Got content-add before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 121, className : "borogove.calls.IncomingProposedSession", methodName : "contentAdd", customParams : [this]});
	}
	contentAccept(_) {
		haxe_Log.trace("Got content-accept before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 125, className : "borogove.calls.IncomingProposedSession", methodName : "contentAccept", customParams : [this]});
	}
	transportInfo(_) {
		haxe_Log.trace("Got transport-info before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 129, className : "borogove.calls.IncomingProposedSession", methodName : "transportInfo", customParams : [this]});
		return thenshim_Promise.resolve(null);
	}
	accept() {
		if(this.accepted) {
...
class borogove_calls_OutgoingProposedSession {
	...
	terminate() {
		haxe_Log.trace("Tried to terminate before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 255, className : "borogove.calls.OutgoingProposedSession", methodName : "terminate", customParams : [this]});
	}
	contentAdd(_) {
		haxe_Log.trace("Got content-add before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 260, className : "borogove.calls.OutgoingProposedSession", methodName : "contentAdd", customParams : [this]});
	}
	contentAccept(_) {
		haxe_Log.trace("Got content-accept before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 265, className : "borogove.calls.OutgoingProposedSession", methodName : "contentAccept", customParams : [this]});
	}
	transportInfo(_) {
		haxe_Log.trace("Got transport-info before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 270, className : "borogove.calls.OutgoingProposedSession", methodName : "transportInfo", customParams : [this]});
		return thenshim_Promise.resolve(null);
	}
	accept() {
		haxe_Log.trace("Tried to accept before initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 276, className : "borogove.calls.OutgoingProposedSession", methodName : "accept", customParams : [this]});
	}
	initiate(stanza) {
...
class borogove_calls_InitiatedSession {
	...
	retract() {
		haxe_Log.trace("Tried to retract session in wrong state: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 383, className : "borogove.calls.InitiatedSession", methodName : "retract", customParams : [this]});
	}
	accept() {
	...
	initiate(stanza) {
		haxe_Log.trace("Trying to initiate already initiated session: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 406, className : "borogove.calls.InitiatedSession", methodName : "initiate"});
		...
		this.setupPeerConnection(function() {
			...
			if(_gthis.afterMedia == null) {
				thenshim_PromiseTools.catch_(_gthis.onPeerConnection(),function(e) {
					haxe_Log.trace("supplyMedia error",{ fileName : "borogove/calls/Session.hx", lineNumber : 595, className : "borogove.calls.InitiatedSession", methodName : "supplyMedia", customParams : [e == null ? "null" : Std.string(e)]});
					_gthis.pc.close();
				});
			} else {
				_gthis.afterMedia();
	...
	setupPeerConnection(callback) {
		...
		this.client.getIceServers(function(servers) {
			...
			_gthis.pc.addEventListener("negotiationneeded",function(event) {
				haxe_Log.trace("renegotiate",{ fileName : "borogove/calls/Session.hx", lineNumber : 614, className : "borogove.calls.InitiatedSession", methodName : "setupPeerConnection", customParams : [event]});
			});
...
class borogove_persistence_Sqlite {
	...
	getChats(accountId) {
		...
		return thenshim_Promise.then(thenshim_Promise.then(this.db.exec("SELECT chat_id, trusted, avatar_sha1, fn, ui_state, blocked, extensions, read_up_to_id, read_up_to_by, notifications_filtered, notify_mention, notify_reply, json(caps) AS caps, caps_ver, json(presence) AS presence, class FROM chats LEFT JOIN caps ON chats.caps_ver=caps.sha1 WHERE account_id=?",[accountId]),function(result) {
			...
			while(_g.current < _g.array.length) {
				...
				capsMap.set(haxe_crypto_Base64.encode(haxe_io_Bytes.ofData(row.sha1)),_gthis.hydrateCaps(json,row.sha1));
			}
			result.caps = null;
			let chats = [];
			let row = null;
	...
	storeMessages(accountId,messages) {
		...
		return thenshim_Promise.then(thenshim_Promise.then(tmp,function(_) {
			...
			while(_g_current < _g_array.length) {
				...
				let tmp5 = x.chatId();
				_g2.push([accountId1,tmp != null ? tmp : "",tmp1 != null ? tmp1 : "",tmp2 != null ? tmp2 : "",tmp4,correctable,tmp5,x.senderId,message.timestamp,message.status,message.direction,message.type,message.asStanza().toString(),message.statusText]);
			}
			let _g3 = [];
	...
	getMessage(accountId,chatId,serverId,localId) {
		let q = "SELECT stanza, direction, type, status, status_text, strftime('%FT%H:%M:%fZ', created_at / 1000.0, 'unixepoch') AS timestamp, sender_id, mam_id, mam_by, sync_point FROM messages WHERE account_id=? AND chat_id=?";
		let params = [accountId,chatId];
		if(serverId != null) {
			q = "SELECT stanza, direction, type, status, status_text, strftime('%FT%H:%M:%fZ', created_at / 1000.0, 'unixepoch') AS timestamp, sender_id, mam_id, mam_by, sync_point FROM messages WHERE account_id=? AND chat_id=?" + " AND mam_id=?";
			params.push(serverId);
		} else if(localId != null) {
			q = "SELECT stanza, direction, type, status, status_text, strftime('%FT%H:%M:%fZ', created_at / 1000.0, 'unixepoch') AS timestamp, sender_id, mam_id, mam_by, sync_point FROM messages WHERE account_id=? AND chat_id=?" + " AND stanza_id=?";
			params.push(localId);
		}
		q += "LIMIT 1";
		let _gthis = this;
	...
	getChatsUnreadDetails(accountId,chats) {
		...
		return thenshim_Promise.then(thenshim_Promise.then(thenshim_Promise.then(thenshim_Promise.resolve(null),function(_) {
			...
			params.push(1);
			let q_b = "";
			q_b = "SELECT chat_id AS chatId, stanza, direction, type, status, status_text, sender_id, mam_id, mam_by, sync_point, CASE WHEN subq.created_at IS NULL THEN COUNT(*) ELSE COUNT(*) - 1 END AS unreadCount, strftime('%FT%H:%M:%fZ', MAX(messages.created_at) / 1000.0, 'unixepoch') AS timestamp FROM messages LEFT JOIN (";
			q_b = "SELECT chat_id AS chatId, stanza, direction, type, status, status_text, sender_id, mam_id, mam_by, sync_point, CASE WHEN subq.created_at IS NULL THEN COUNT(*) ELSE COUNT(*) - 1 END AS unreadCount, strftime('%FT%H:%M:%fZ', MAX(messages.created_at) / 1000.0, 'unixepoch') AS timestamp FROM messages LEFT JOIN (" + Std.string(subq_b);
	...
	updateMessageStatus(accountId,localId,status,statusText) {
		let _gthis = this;
		return thenshim_Promise.then(thenshim_Promise.then(thenshim_Promise.then(thenshim_Promise.then(this.db.exec("UPDATE messages SET status=?, status_text=? WHERE account_id=? AND stanza_id=? AND direction=? AND status <> ? AND status <> ?",[status,statusText,accountId,localId,1,2,3]),function(_) {
			return _gthis.db.exec("SELECT stanza, direction, type, status, status_text, strftime('%FT%H:%M:%fZ', created_at / 1000.0, 'unixepoch') AS timestamp, sender_id, correction_id AS stanza_id, mam_id, mam_by, sync_point FROM messages WHERE account_id=? AND stanza_id=? AND direction=? LIMIT 1",[accountId,localId,1]);
		}),function(result) {
	...
	hydrateReplyTo(accountId,messages,replyTos) {
		...
		if(replyTos.length < 1) {
			...
			let params = [accountId];
			let q_b = "";
			q_b = "SELECT chat_id, stanza_id, stanza, direction, type, status, status_text, strftime('%FT%H:%M:%fZ', created_at / 1000.0, 'unixepoch') AS timestamp, sender_id, mam_id, mam_by, sync_point FROM messages WHERE account_id=? AND (";
			...
			q_b = "SELECT chat_id, stanza_id, stanza, direction, type, status, status_text, strftime('%FT%H:%M:%fZ', created_at / 1000.0, 'unixepoch') AS timestamp, sender_id, mam_id, mam_by, sync_point FROM messages WHERE account_id=? AND (" + Std.string(result.join(" OR "));
			q_b += ")";
	...
	hydrateCaps(o,ver) {
		...
		while(_g2 < _g3) {
			let i = _g2++;
			result1[i] = borogove_Stanza.parse(_this1[i]);
		}
		return new borogove_Caps(o1,result,tmp2,result1,ver);
	}
}
...
class borogove_streams_XmppJsStream extends borogove_GenericStream {
	...
	register(domain,preAuth) {
		...
		return new Promise(function(resolve,reject) {
			...
			_gthis.client.on("stanza",function(stanza) {
				_gthis.onStanza(borogove_streams_XmppJsStream.convertToStanza(stanza,_gthis.client.NS));
			});
			_gthis.client.start().catch(function(err) {
				haxe_Log.trace(err,{ fileName : "borogove/streams/XmppJsStream.hx", lineNumber : 226, className : "borogove.streams.XmppJsStream", methodName : "register"});
				reject(err);
			});
		});
	}
	connect(jidS,sm) {
		...
		xmpp.start().catch(function(err) {
			...
			if((xmppError != null ? xmppError.name : null) == "SASLError") {
				...
				haxe_Log.trace(err,{ fileName : "borogove/streams/XmppJsStream.hx", lineNumber : 362, className : "borogove.streams.XmppJsStream", methodName : "connect"});
			}
		});
	}
	disconnect() {
	...
	triggerSMupdate() {
		let tmp;
		if(this.client != null) {
			let tmp1 = this.client.streamManagement;
			tmp = !(tmp1 != null ? tmp1.enabled : null);
		} else {
			tmp = true;
		}
		if(tmp || !this.emitSMupdates) {
			return;
		}
		...
		let tmp1 = this.client.streamManagement.outbound_q;
	...
	static convertToStanza(el,xmlns) {
		let tmp = el.attrs;
		let attrs = tmp != null ? tmp : { };
		if(attrs["xmlns"] == null && xmlns != null) {
			attrs["xmlns"] = xmlns;
		}
		let stanza = new borogove_Stanza(el.name,attrs);
		let _g = 0;
		let _g1 = el.children;
		while(_g < _g1.length) {
			let child = _g1[_g];
...
borogove_Version.HUMAN = "fc76764";
