...
class borogove_persistence_Sqlite {
        ...
        this.db = new borogove_persistence_SqliteDriver(dbfile, function (exec) {
            return borogove.thenshim_Promise.then(exec(["PRAGMA user_version"]), function (iter) {
                ...
                return borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove.thenshim_Promise.resolve(null), function (_) {
                    ...
                    if (version < 9) {
                        return exec(["ALTER TABLE chats ADD COLUMN meta BLOB NOT NULL DEFAULT jsonb('{}')", "PRAGMA user_version = 9"]);
                    }
                    return borogove.thenshim_Promise.resolve(null);
                });
            });
    ...
    storeChats(accountId, chats) {
        ...
        this.storeChatTimer = borogove.haxe_Timer.delay(function () {
            ...
            while (x.hasNext()) {
                ...
                _g.push([accountId1, x2, row, x3, row1, x4, x5, row2, x6, x7, row3, row4, row5, row6, row7, row8, x8, JSON.stringify({ status: row9, threads: t })]);
            }
            let _g1 = [];
    ...
    getMessages(accountId, chatId, sortId, op) {
        let q = "WITH page AS (SELECT stanza_id, mam_id FROM messages where account_id=? AND chat_id=? AND (stanza_id IS NULL OR stanza_id='' OR stanza_id=correction_id)";
        let params = [accountId, chatId];
        if (sortId != null) {
            q = "WITH page AS (SELECT stanza_id, mam_id FROM messages where account_id=? AND chat_id=? AND (stanza_id IS NULL OR stanza_id='' OR stanza_id=correction_id)" + (" AND messages.sort_id " + op + " ?");
            params.push(sortId);
        }
    ...
    getMessagesBefore(accountId, chatId, before) {
        return this.getMessages(accountId, chatId, before != null ? before.sortId : null, "<");
    }
    getMessagesAfter(accountId, chatId, after) {
        return this.getMessages(accountId, chatId, after != null ? after.sortId : null, ">");
    }
    getMessagesAround(accountId, around) {
        let chatId = around.chatId();
        return borogove.thenshim_Promise.then(borogove.thenshim_PromiseTools.all([this.getMessages(accountId, chatId, around.sortId, "<"), this.getMessages(accountId, chatId, around.sortId, ">=")]), function (results) {
            let _g = [];
            let _g_current = 0;
...
class borogove_persistence_SqliteDriver {
    constructor(dbfile, migrate) {
        let _gthis = this;
        borogove.thenshim_Promise.then(borogove.thenshim_Promise.then(borogove_persistence_Worker1.v2({ worker: function () {
            ...
            return migrate(function (sql) {
                ...
                return _gthis1.execMany(result);
            });
        });
    }
    execMany(qs) {
        ...
        if (this.sqlite == null || this.dbId == null) {
            return borogove.thenshim_Promise.then(borogove.thenshim_Promise._new(function (resolve, reject) {
                borogove.haxe_Timer.delay(function () {
                    resolve(null);
                }, 100);
            }), function (_) {
                return _gthis.execMany(qs);
            });
        }
        let first = qs.shift();
