		...
		sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 1:
					...
					chatMessages.push(_gthis.prepareIncomingMessage(message,new snikket_Stanza("message",{ from : message.senderId()})));
					break;
				case 2:
					let action = m.action;
...
class snikket_DirectChat extends snikket_Chat {
	...
	correctMessage(localId,message) {
		...
		let _gthis = this;
		this.client.storeMessages([message],function(corrected) {
	...
	sendMessage(message) {
		...
		case 1:
			...
			this.client.storeMessages([message],function(stored) {
				let _g = 0;
				let _g1 = message.recipients;
				while(_g < _g1.length) {
					let recipient = _g1[_g];
		...
		case 3:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 796, className : "snikket.DirectChat", methodName : "sendMessage", customParams : [fromStanza]});
...
class snikket_Channel extends snikket_Chat {
	...
	doSync(lastId) {
		...
		this.sync.onError(function(stanza) {
			...
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC failed",{ fileName : "snikket/Chat.hx", lineNumber : 1107, className : "snikket.Channel", methodName : "doSync", customParams : [_gthis.chatId,stanza]});
			}
		});
		this.sync.fetchNext();
	}
	...
	sendMessage(message) {
		...
		case 3:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 1297, className : "snikket.Channel", methodName : "sendMessage", customParams : [fromStanza]});
			throw haxe_Exception.thrown("Trying to send invalid message.");
		}
	}
	removeReaction(m,reaction) {
...
class snikket_ChatMessage {
	...
	makeModerated(timestamp,moderatorId,reason) {
		this.text = null;
		this.attachments = [];
		this.payloads = [];
		this.versions = [];
		let cleanedStub = this.clone();
		...
		let head = this.clone();
		head.set_timestamp(timestamp);
		this.versions = [head,cleanedStub];
	}
	set_localId(localId) {
...
class snikket_Client extends snikket_EventEmitter {
	...
	_hx_constructor(address,persistence) {
			...
			if(jmiPro != null && jmiPro.attr["id"] != null) {
				...
				if(session != null) {
					try {
						...
						haxe_Log.trace("JMI proceed failed",{ fileName : "snikket/Client.hx", lineNumber : 240, className : "snikket.Client", methodName : "new", customParams : [e]});
					}
				}
			}
		...
		this.stream.onIq(snikket_IqRequestType.Set,"jingle","urn:xmpp:jingle:1",function(stanza) {
			...
			if(jingle.attr["action"] == "session-initiate") {
				if(session != null) {
					try {
						...
						let e = haxe_Exception.caught(_g);
						haxe_Log.trace("Bad session-inititate",{ fileName : "snikket/Client.hx", lineNumber : 347, className : "snikket.Client", methodName : "new", customParams : [e]});
						chat.jingleSessions.delete(session.get_sid());
					}
				} else {
			...
			if(session != null && jingle.attr["action"] == "session-accept") {
				try {
					...
					let e = haxe_Exception.caught(_g);
					haxe_Log.trace("session-accept failed",{ fileName : "snikket/Client.hx", lineNumber : 362, className : "snikket.Client", methodName : "new", customParams : [e]});
				}
			}
		...
		this.stream.on("presence",function(event) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == "unavailable") {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "snikket/Client.hx", lineNumber : 533, className : "snikket.Client", methodName : "new"});
					return snikket_EventResult.EventUnhandled;
				}
		...
		this.bookmarksGet(function() {
			...
			_gthis.sync(function(syncFinished) {
				...
				_gthis.persistence.getChatsUnreadDetails(_gthis.accountId(),_gthis.chats,function(details) {
					...
					haxe_Log.trace("SYNC: done",{ fileName : "snikket/Client.hx", lineNumber : 722, className : "snikket.Client", methodName : "onConnected"});
				});
			});
		});
		...
		this.persistence.findServicesWithFeature(this.accountId(),"urn:xmpp:http:upload:0",function(services) {
			...
			tink_io_Source.chunked(new tink_io_js_BlobSource(source.name,source,0,chunkSize)).forEach(tink_streams_Handler.ofSafeSync(function(chunk) {
				...
				if(o._hx_index == 3) {
					...
					haxe_Log.trace("Error computing attachment hash",{ fileName : "snikket/Client.hx", lineNumber : 745, className : "snikket.Client", methodName : "prepareAttachment", customParams : [o]});
					callback(null);
				}
			});
		});
		...
		window.crypto.subtle.exportKey("raw",vapid_key.publicKey).then(function(vapid_public_raw) {
			return reg.pushManager.subscribe({ userVisibleOnly : true, applicationServerKey : vapid_public_raw}).then(function(pushSubscription) {
				if(pushSubscription == null) {
					haxe_Log.trace("WebPush subscription failed",{ fileName : "snikket/Client.hx", lineNumber : 947, className : "snikket.Client", methodName : "subscribePush"});
					return;
				}
	...
	doSync(callback,lastId) {
		...
		sync.onMessages(function(messageList) {
			...
			haxe_Log.trace("SYNC: MAM page wait for writes",{ fileName : "snikket/Client.hx", lineNumber : 1461, className : "snikket.Client", methodName : "doSync"});
			thenshim_Promise.then(thenshim_PromiseTools.all(promises),function(results) {
				...
				haxe_Log.trace("SYNC: error",{ fileName : "snikket/Client.hx", lineNumber : 1483, className : "snikket.Client", methodName : "doSync", customParams : [e == null ? "null" : Std.string(e)]});
				callback(false);
			});
		});
		sync.onError(function(stanza) {
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC: error",{ fileName : "snikket/Client.hx", lineNumber : 1492, className : "snikket.Client", methodName : "doSync", customParams : [stanza]});
				if(callback != null) {
					callback(false);
				}
			}
...
class snikket_Date {
	static format(d) {
		return Std.string(d.getUTCFullYear()) + "-" + StringTools.lpad(Std.string(d.getUTCMonth() + 1),"0",2) + "-" + StringTools.lpad(Std.string(d.getUTCDate()),"0",2) + "T" + StringTools.lpad(Std.string(d.getUTCHours()),"0",2) + ":" + StringTools.lpad(Std.string(d.getUTCMinutes()),"0",2) + ":" + StringTools.lpad(Std.string(d.getUTCSeconds()),"0",2) + ".000Z";
	}
}
...
class snikket_Message {
	...
	static fromStanza(stanza,localJid,inputTimestamp) {
		...
		if(moderated != null && moderateServerId != null && isGroupchat && msg.from != null && msg.from.isBare() && msg.from.asString() == msg.chatId()) {
			...
			return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.ModerateMessageStanza(new snikket_ModerationAction(msg.chatId(),moderateServerId,timestamp,by,reason)));
		}
		...
		if(msg.text == null && msg.attachments.length < 1 && msg.versions.length < 1) {
			return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.UnknownMessageStanza(stanza));
		}
		let _g8 = 0;
...
class snikket_MessageSync {
	...
	fetchNext() {
		...
		query.onFinished(function() {
			...
			if(result == null) {
				haxe_Log.trace("Error from MAM, stopping sync",{ fileName : "snikket/MessageSync.hx", lineNumber : 102, className : "snikket.MessageSync", methodName : "fetchNext"});
...
class snikket_persistence_Sqlite {
	...
	storeMessages(accountId,messages,callback) {
		...
		thenshim_Promise.then(tmp,function(_) {
			let _gthis1 = _gthis.db;
			...
			while(_g_current < _g_array.length) {
				...
				_g2.push([accountId,x.serverId,x.serverIdBy,x.localId,x.syncPoint,x.chatId(),x.timestamp,x.status,x.direction,x.asStanza().toString()]);
			}
			let _g3 = [];
			...
			return thenshim_Promise.then(_gthis1.exec(tmp,_g3),function(_) {
				callback(messages);
			});
		});
	}
	...
	getMessage(accountId,chatId,serverId,localId,callback) {
		let q = "SELECT stanza, direction, strftime('%FT%H:%M:%fZ', created_at / 1000.0, 'unixepoch') AS timestamp, mam_id, mam_by FROM messages WHERE account_id=? AND chat_id=?";
		...
		q += "LIMIT 1";
		let _gthis = this;
		thenshim_Promise.then(this.db.exec(q,params),function(result) {
			let _g = result;
			while(_g.current < _g.array.length) {
				let row = _g.array[_g.current++];
				callback(_gthis.hydrateMessage(accountId,row));
				return;
			}
			callback(null);
		});
	...
	getMessages(accountId,chatId,time,op) {
		...
		return thenshim_Promise.then(thenshim_Promise.then(this.db.exec(q,params),function(result) {
			return { hasNext : $bind(result,result.hasNext), next : function() {
				return _gthis.hydrateMessage(accountId,result.array[result.current++]);
			}};
		}),function(iter) {
			if(op == "<" || op == "<=") {
				let arr = Lambda.array({ iterator : function() {
					return iter;
				}});
				arr.reverse();
				let reviter = new haxe_iterators_ArrayIterator(arr);
				return { hasNext : $bind(reviter,reviter.hasNext), next : $bind(reviter,reviter.next)};
			} else {
				return iter;
			}
		});
	...
	getMessagesBefore(accountId,chatId,beforeId,beforeTime,callback) {
		thenshim_Promise.then(this.getMessages(accountId,chatId,beforeTime,"<"),function(iter) {
			callback(Lambda.array({ iterator : function() {
				return iter;
			}}));
		});
	}
	getMessagesAfter(accountId,chatId,afterId,afterTime,callback) {
		thenshim_Promise.then(this.getMessages(accountId,chatId,afterTime,">"),function(iter) {
			callback(Lambda.array({ iterator : function() {
				return iter;
			}}));
		});
	...
	getMessagesAround(accountId,chatId,aroundId,aroundTime,callback) {
		...
		thenshim_Promise.then(thenshim_Promise.then(aroundTime == null ? thenshim_Promise.then(thenshim_Promise._new(function(resolve,reject) {
			...
			while(_g_current < _g_array.length) {
				let x = _g_array[_g_current++];
				let iter = x;
				_g.push(Lambda.array({ iterator : function() {
					return iter;
				}}));
			}
			let _g1 = [];
	...
	getChatsUnreadDetails(accountId,chats,callback) {
		...
		let params = [accountId];
		let subq_b = "";
		subq_b += "SELECT chat_id, MAX(ROWID) AS row FROM messages WHERE account_id=?";
		...
		params.push(1);
		let q_b = "";
		q_b += "SELECT chat_id AS chatId, stanza, direction, mam_id, mam_by, CASE WHEN subq.row IS NULL THEN COUNT(*) ELSE COUNT(*) - 1 END AS unreadCount, strftime('%FT%H:%M:%fZ', MAX(messages.created_at) / 1000.0, 'unixepoch') AS timestamp FROM messages LEFT JOIN (";
		...
		q_b += ") AND (subq.row IS NULL OR messages.ROWID >= subq.row) GROUP BY chat_id;";
		let _gthis = this;
		thenshim_Promise.then(this.db.exec(q_b,params),function(result) {
			...
			while(_g.current < _g.array.length) {
				let row = _g.array[_g.current++];
				details.push({ unreadCount : row.unreadCount, chatId : row.chatId, message : _gthis.hydrateMessage(accountId,row)});
			}
			callback(details);
		});
	}
	...
	updateMessageStatus(accountId,localId,status,callback) {
		let _gthis = this;
		thenshim_Promise.then(thenshim_Promise.then(this.db.exec("UPDATE messages SET status=? WHERE account_id=? AND stanza_id=? AND direction=? AND status <> ?",[status,accountId,localId,1,2]),function(_) {
			return _gthis.db.exec("SELECT stanza, strftime('%FT%H:%M:%fZ') AS timestamp, mam_id, mam_by FROM messages WHERE account_id=? AND stanza_id=? AND direction=?",[accountId,localId,1]);
		}),function(result) {
			let _g = result;
			while(_g.current < _g.array.length) {
				let row = _g.array[_g.current++];
	...
	hydrateMessage(accountId,row) {
		let accountJid = snikket_JID.parse(accountId);
		let x = snikket_ChatMessage.fromStanza(snikket_Stanza.parse(row.stanza),snikket_JID.parse(accountId));
		x.set_timestamp(row.timestamp);
		x.direction = row.direction;
		x.set_serverId(row.mam_id);
