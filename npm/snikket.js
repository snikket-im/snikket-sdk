var exports = {};
try { global.require = createRequire(import.meta.url); } catch (e) {  }
...
class snikket_Caps {
	...
	verRaw() {
		...
		return haxe_crypto_Sha1.make(haxe_io_Bytes.ofString(s));
	}
	ver() {
		return haxe_crypto_Base64.encode(this.verRaw(),true);
	}
...
class snikket_Chat {
	constructor(client,stream,persistence,chatId,uiState,extensions,readUpToId,readUpToBy) {
		if(snikket_Chat._hx_skip_constructor) {
			return;
		}
		this._hx_constructor(client,stream,persistence,chatId,uiState,extensions,readUpToId,readUpToBy);
	}
	_hx_constructor(client,stream,persistence,chatId,uiState,extensions,readUpToId,readUpToBy) {
		if(uiState == null) {
			uiState = 1;
		}
		...
		sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 1:
					...
					let chatMessage = _gthis.prepareIncomingMessage(message,new snikket_Stanza("message",{ from : message.senderId()}));
					promises.push(thenshim_Promise._new(function(resolve,reject) {
						_gthis.persistence.storeMessage(_gthis.client.accountId(),chatMessage,resolve);
					}));
					break;
				case 2:
					let update = m.update;
			...
			thenshim_Promise.then(thenshim_PromiseTools.all(promises),function(chatMessages) {
	...
	updateFromBookmark(item) {
		...
		if(fn != null) {
			this.setDisplayName(fn);
		}
		this.uiState = conf.attr["autojoin"] == "1" || conf.attr["autojoin"] == "true" ? 1 : 2;
	...
	preview() {
		let tmp = this.lastMessage;
		let tmp1 = tmp != null ? tmp.text : null;
		if(tmp1 != null) {
			return tmp1;
		} else {
			return "";
	...
	markReadUpToId(upTo,upToBy,callback) {
		...
		this.persistence.storeChat(this.client.accountId(),this);
		let _gthis = this;
		this.persistence.getMessagesBefore(this.client.accountId(),this.chatId,null,null,function(messages) {
			let i = messages.length;
			while(--i >= 0) if(messages[i].serverId == _gthis.readUpToId) {
				break;
			}
			if(i > 0) {
				_gthis._unreadCount = messages.length - (i + 1);
			}
			if(callback != null) {
				callback();
			}
...
class snikket_DirectChat extends snikket_Chat {
	...
	getParticipantDetails(participantId) {
		...
		return new snikket_Participant(chat.getDisplayName(),chat.getPhoto(),chat.getPlaceholder());
	}
	...
	prepareOutgoingMessage(message) {
		...
		message.set_timestamp(tmp != null ? tmp : snikket_Date.format(new Date()));
		message.direction = 1;
	...
	correctMessage(localId,message) {
		let toSend = this.prepareOutgoingMessage(message.clone());
		message = this.prepareOutgoingMessage(message);
		message.resetLocalId();
		message.versions = [toSend];
		message.set_localId(localId);
		let _gthis = this;
		this.persistence.storeMessage(this.client.accountId(),message,function(corrected) {
			...
			while(_g < _g1.length) {
				...
				_gthis.client.sendStanza(toSend.asStanza());
			}
			let tmp = _gthis.lastMessage;
			if(localId == (tmp != null ? tmp.localId : null)) {
				_gthis.setLastMessage(corrected);
				_gthis.client.trigger("chats/update",[_gthis]);
			}
			_gthis.client.notifyMessageHandlers(corrected);
		});
	}
	sendMessage(message) {
		if(this.typingTimer != null) {
		...
		message = this.prepareOutgoingMessage(message);
		let fromStanza = snikket_Message.fromStanza(message.asStanza(),this.client.jid).parsed;
		let _gthis = this;
		switch(fromStanza._hx_index) {
		case 1:
			let _g = fromStanza.message;
			this.persistence.storeMessage(this.client.accountId(),message,function(stored) {
				let _g = 0;
				let _g1 = message.recipients;
				while(_g < _g1.length) {
					let recipient = _g1[_g];
				...
				_gthis.setLastMessage(message);
				_gthis.client.trigger("chats/update",[_gthis]);
				_gthis.client.notifyMessageHandlers(stored);
			});
			break;
		case 2:
			...
			this.persistence.storeReaction(this.client.accountId(),update,function(stored) {
				...
				while(_g < _g1.length) {
					...
					++_g;
					message.to = recipient;
					_gthis.client.sendStanza(message.asStanza());
				}
				if(stored != null) {
					_gthis.client.notifyMessageHandlers(stored);
				}
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 700, className : "snikket.DirectChat", methodName : "sendMessage", customParams : [fromStanza]});
	...
	removeReaction(m,reaction) {
		...
		while(!_g_lastStep.done) {
			...
			let senders = _g_value;
			let tmp;
			if(areaction != reaction) {
				let x = this.client.accountId();
				tmp = senders.includes(x);
			} else {
				tmp = false;
			}
			if(tmp) {
				reactions.push(areaction);
			}
		}
		let update = new snikket_ReactionUpdate(snikket_ID.long(),null,m.localId,m.chatId(),snikket_Date.format(new Date()),this.client.accountId(),reactions);
		let _gthis = this;
		this.persistence.storeReaction(this.client.accountId(),update,function(stored) {
	...
	markReadUpTo(message) {
		if(this.readUpTo() == message.localId || this.readUpTo() == message.serverId) {
			return;
		}
		let tmp = message.localId;
		let upTo = tmp != null ? tmp : message.serverId;
		if(upTo == null) {
			return;
		}
		if(message.isIncoming()) {
			...
			while(_g < _g1.length) {
				let recipient = _g1[_g];
				++_g;
				let stanza = new snikket_Stanza("message",{ to : recipient, id : snikket_ID.long()}).tag("displayed",{ xmlns : "urn:xmpp:chat-markers:0", id : upTo}).up();
				if(message.threadId != null) {
					stanza.textTag("thread",message.threadId);
				}
				this.client.sendStanza(stanza);
			}
		}
		let _gthis = this;
		this.markReadUpToId(message.serverId,message.serverIdBy,function() {
			_gthis.publishMds();
			_gthis.client.trigger("chats/update",[_gthis]);
		});
	}
	bookmark() {
		let _gthis = this;
		this.stream.sendIq(new snikket_Stanza("iq",{ type : "set"}).tag("query",{ xmlns : "jabber:iq:roster"}).tag("item",{ jid : this.chatId}).up().up(),function(response) {
			if(response.attr["type"] == "error") {
				return;
			}
...
class snikket_Channel extends snikket_Chat {
	constructor(client,stream,persistence,chatId,uiState,extensions,readUpToId,readUpToBy,disco) {
		snikket_Chat._hx_skip_constructor = true;
		super();
		snikket_Chat._hx_skip_constructor = false;
		this._hx_constructor(client,stream,persistence,chatId,uiState,extensions,readUpToId,readUpToBy,disco);
	}
	_hx_constructor(client,stream,persistence,chatId,uiState,extensions,readUpToId,readUpToBy,disco) {
		if(uiState == null) {
			uiState = 1;
		}
		this.inSync = true;
		this.disco = new snikket_Caps("",[],["http://jabber.org/protocol/muc"]);
		super._hx_constructor(client,stream,persistence,chatId,uiState,extensions,readUpToId,readUpToBy);
		if(disco != null) {
			this.disco = disco;
		}
	}
	selfPing(shouldRefreshDisco) {
		if(shouldRefreshDisco == null) {
			shouldRefreshDisco = true;
		}
		if(this.uiState == 2) {
		...
		let _gthis = this;
		this.stream.sendIq(new snikket_Stanza("iq",{ type : "get", to : this.getFullJid().asString()}).tag("ping",{ xmlns : "urn:xmpp:ping"}).up(),function(response) {
			if(response.attr["type"] == "error") {
				let tmp = response.getChild("error");
				...
				(shouldRefreshDisco ? $bind(_gthis,_gthis.refreshDisco) : function(cb) {
					cb();
				})(function() {
	...
	doSync(lastId) {
		...
		let sync = new snikket_MessageSync(this.client,this.stream,lastId == null ? { startTime : threeDaysAgo} : { page : { after : lastId}},this.chatId);
		...
		sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 1:
					...
					promises.push(thenshim_Promise._new(function(resolve,reject) {
						_gthis.persistence.storeMessage(_gthis.client.accountId(),message,resolve);
					}));
					if(message.chatId() == _gthis.chatId) {
				...
				case 2:
					let update = m.update;
					promises.push(thenshim_Promise._new(function(resolve,reject) {
						_gthis.persistence.storeReaction(_gthis.client.accountId(),update,resolve);
					}));
					break;
				default:
				}
			...
			if(sync.hasMore()) {
				...
				thenshim_Promise.then(thenshim_PromiseTools.all(promises),function(_) {
					_gthis.inSync = true;
					...
					let readIndex = Lambda.findIndex(chatMessages,function(m) {
						return m.serverId == _gthis.readUpTo();
					});
					if(readIndex < 0) {
						_gthis.setUnreadCount(_gthis.unreadCount() + chatMessages.length);
					} else {
						_gthis.setUnreadCount(chatMessages.length - readIndex - 1);
					}
	...
	refreshDisco(callback) {
		...
		discoGet.onFinished(function() {
			if(discoGet.getResult() != null) {
				...
				_gthis.persistence.storeChat(_gthis.client.accountId(),_gthis);
			}
			if(callback != null) {
				callback();
			}
	...
	preview() {
		...
		return this.lastMessage.sender.resource + ": " + super.preview();
	}
	livePresence() {
		let jsIterator = this.presence.entries();
		let _g_jsIterator = jsIterator;
		...
		while(!_g_lastStep.done) {
			...
			let p = _g_value;
			let _g = 0;
			let tmp = p != null ? p.mucUser : null;
			let tmp1 = tmp != null ? tmp.allTags("status") : null;
			let _g1 = tmp1 != null ? tmp1 : [];
			while(_g < _g1.length) {
				let status = _g1[_g];
				++_g;
				if(status.attr["code"] == "110") {
					return true;
				}
			}
		}
	...
	getParticipantDetails(participantId) {
		if(participantId == this.getFullJid().asString()) {
			...
			return new snikket_Participant(this.client.displayName(),chat.getPhoto(),chat.getPlaceholder());
		} else {
			...
			return new snikket_Participant(nick,null,placeholderUri);
		}
	}
	getMessagesBefore(beforeId,beforeTime,handler) {
		let _gthis = this;
	...
	prepareIncomingMessage(message,stanza) {
		...
		if(message.senderId() == this.getFullJid().asString()) {
	...
	sendMessage(message) {
		...
		case 2:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 1116, className : "snikket.Channel", methodName : "sendMessage", customParams : [fromStanza]});
			throw haxe_Exception.thrown("Trying to send invalid message.");
		}
	}
	removeReaction(m,reaction) {
		...
		while(!_g_lastStep.done) {
			...
			let tmp;
			if(areaction != reaction) {
				let x = this.getFullJid().asString();
				tmp = senders.includes(x);
			} else {
				tmp = false;
			}
		...
		let update = new snikket_ReactionUpdate(snikket_ID.long(),m.serverId,null,m.chatId(),snikket_Date.format(new Date()),this.client.accountId(),reactions);
		let _gthis = this;
		this.persistence.storeReaction(this.client.accountId(),update,function(stored) {
			let stanza = update.asStanza();
	...
	markReadUpTo(message) {
		...
		let upTo = message.serverId;
		if(upTo == null) {
			return;
		}
		let stanza = new snikket_Stanza("message",{ to : this.chatId, id : snikket_ID.long(), type : "groupchat"}).tag("displayed",{ xmlns : "urn:xmpp:chat-markers:0", id : upTo}).up();
		if(message.threadId != null) {
			stanza.textTag("thread",message.threadId);
		}
		this.client.sendStanza(stanza);
		let _gthis = this;
		this.markReadUpToId(upTo,message.serverIdBy,function() {
			_gthis.publishMds();
			_gthis.client.trigger("chats/update",[_gthis]);
		});
	}
	...
	sendChatState(state,threadId) {
		let stanza = new snikket_Stanza("message",{ id : snikket_ID.long(), type : "chat", from : this.client.jid.asString(), to : this.chatId}).tag(state,{ xmlns : "http://jabber.org/protocol/chatstates"}).up();
		if(threadId != null) {
			stanza.textTag("thread",threadId);
		}
		this.stream.sendStanza(stanza);
...
class snikket_SerializedChat {
	constructor(chatId,trusted,avatarSha1,presence,displayName,uiState,extensions,readUpToId,readUpToBy,disco,klass) {
		this.chatId = chatId;
		this.trusted = trusted;
		...
		let tmp1 = extensions;
		this.extensions = tmp1 != null ? tmp1 : "<extensions xmlns='urn:app:bookmarks:1' />";
		this.readUpToId = readUpToId;
		this.readUpToBy = readUpToBy;
		this.disco = disco;
		this.klass = klass;
	...
	toChat(client,stream,persistence) {
		...
		if(this.klass == "DirectChat") {
			chat = new snikket_DirectChat(client,stream,persistence,this.chatId,this.uiState,extensionsStanza,this.readUpToId,this.readUpToBy);
		} else if(this.klass == "Channel") {
			let channel = new snikket_Channel(client,stream,persistence,this.chatId,this.uiState,extensionsStanza,this.readUpToId,this.readUpToBy);
			let tmp = this.disco;
			channel.disco = tmp != null ? tmp : new snikket_Caps("",[],["http://jabber.org/protocol/muc"]);
			chat = channel;
		} else {
			throw haxe_Exception.thrown("Unknown class: " + this.klass);
		}
		if(this.displayName != null) {
			chat.setDisplayName(this.displayName);
		}
...
class snikket_ChatMessage {
	...
	reply() {
		let m = new snikket_ChatMessage();
		m.isGroupchat = this.isGroupchat;
	...
	set_localId(localId) {
		if(this.localId != null) {
			throw new haxe_Exception("Message already has a localId set");
		}
		return this.localId = localId;
	}
	set_serverId(serverId) {
		if(this.serverId != null && this.serverId != serverId) {
			throw new haxe_Exception("Message already has a serverId set");
		}
	...
	html() {
		...
		while(_g1 < _g2.length) {
			let v = _g2[_g1];
			++_g1;
			if(v.attr["xmlns"] == "urn:xmpp:fallback:0" && (v.attr["for"] == "jabber:x:oob" || v.attr["for"] == "urn:xmpp:sims:1" || _gthis.replyToMessage != null && v.attr["for"] == "urn:xmpp:reply:0" || v.attr["for"] == "http://jabber.org/protocol/address")) {
				_g.push(v);
			}
		}
		let _this = _g;
	...
	senderId() {
		let tmp = this.sender;
		let tmp1 = tmp != null ? tmp.asString() : null;
		if(tmp1 != null) {
			return tmp1;
		} else {
	...
	asStanza() {
		let body = this.text;
		let attrs = { type : this.isGroupchat ? "groupchat" : "chat"};
		if(this.from != null) {
		...
		if(replyToM != null) {
			...
			body = quoteText + body;
			let replyId = replyToM.isGroupchat ? replyToM.serverId : replyToM.localId;
			if(replyId != null) {
				...
				if(reaction != null) {
					...
					while(!_g_lastStep.done) {
						...
						let senders = _g_value;
						let tmp;
						let tmp1 = addedReactions.get(areaction);
						if(!(tmp1 != null && tmp1)) {
							let x = this.senderId();
							tmp = senders.includes(x);
						} else {
				...
				let tmp1 = replyToM.from;
				stanza.tag("reply",{ xmlns : "urn:xmpp:reply:0", to : tmp1 != null ? tmp1.asString() : null, id : replyId}).up();
			}
		}
		let _g = 0;
	...
	clone() {
		let cls = js_Boot.getClass(this);
		let inst = Object.create(cls.prototype);
		let fields = Type.getInstanceFields(cls);
		let _g = 0;
		while(_g < fields.length) {
			let field = fields[_g];
			++_g;
			let val = Reflect.field(this,field);
	...
	static fromStanza(stanza,localJid) {
		let _g = snikket_Message.fromStanza(stanza,localJid).parsed;
		if(_g._hx_index == 1) {
...
class snikket_Client extends snikket_EventEmitter {
	...
	_hx_constructor(address,persistence) {
		...
		this.stream.on("sm/update",function(data) {
			persistence.storeStreamManagement(_gthis.accountId(),data.sm);
			return snikket_EventResult.EventHandled;
		});
		this.stream.on("sm/ack",function(data) {
			persistence.updateMessageStatus(_gthis.accountId(),data.id,1,$bind(_gthis,_gthis.notifyMessageHandlers));
			return snikket_EventResult.EventHandled;
		});
		this.stream.on("sm/fail",function(data) {
			persistence.updateMessageStatus(_gthis.accountId(),data.id,3,$bind(_gthis,_gthis.notifyMessageHandlers));
			return snikket_EventResult.EventHandled;
		});
		this.stream.on("message",function(event) {
			...
			if(jmiPro != null && jmiPro.attr["id"] != null) {
				...
				if(session != null) {
					try {
						...
						haxe_Log.trace("JMI proceed failed",{ fileName : "snikket/Client.hx", lineNumber : 186, className : "snikket.Client", methodName : "new", customParams : [e]});
					}
				}
			}
			...
			let message = snikket_Message.fromStanza(stanza,_gthis.jid);
			...
			case 1:
				...
				if(chat == null && stanza.attr["type"] != "groupchat") {
					chat = _gthis.getDirectChat(chatMessage.chatId());
				}
				if(chat != null) {
					let updateChat = function(chatMessage) {
						...
						_gthis.notifyMessageHandlers(chatMessage);
					};
					chatMessage = chat.prepareIncomingMessage(chatMessage,stanza);
					if(chatMessage.serverId == null) {
						updateChat(chatMessage);
					} else {
						persistence.storeMessage(_gthis.accountId(),chatMessage,updateChat);
					}
				}
				break;
			case 2:
				...
				persistence.storeReaction(_gthis.accountId(),update,function(stored) {
					if(stored != null) {
						_gthis.notifyMessageHandlers(stored);
					}
				});
				break;
			default:
			...
			if(pubsubEvent != null && pubsubEvent.getFrom() != null && pubsubEvent.getNode() == "urn:xmpp:avatar:metadata" && pubsubEvent.getItems().length > 0) {
				...
				let avatarSha1 = haxe_io_Bytes.ofHex(avatarSha1Hex).b.bufferValue;
				...
				persistence.storeChat(_gthis.accountId(),chat);
		...
		this.stream.onIq(snikket_IqRequestType.Set,"jingle","urn:xmpp:jingle:1",function(stanza) {
			...
			if(jingle.attr["action"] == "session-initiate") {
				if(session != null) {
					try {
						...
						let e = haxe_Exception.caught(_g);
						haxe_Log.trace("Bad session-inititate",{ fileName : "snikket/Client.hx", lineNumber : 318, className : "snikket.Client", methodName : "new", customParams : [e]});
						chat.jingleSessions.delete(session.get_sid());
					}
				} else {
			...
			if(session != null && jingle.attr["action"] == "session-accept") {
				try {
					...
					let e = haxe_Exception.caught(_g);
					haxe_Log.trace("session-accept failed",{ fileName : "snikket/Client.hx", lineNumber : 333, className : "snikket.Client", methodName : "new", customParams : [e]});
				}
			}
		...
		this.stream.on("presence",function(event) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == null) {
				...
				let chat = _gthis.getChat(from.asBare().asString());
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "snikket/Client.hx", lineNumber : 394, className : "snikket.Client", methodName : "new"});
					return snikket_EventResult.EventUnhandled;
				}
				if(c == null) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == "unavailable") {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "snikket/Client.hx", lineNumber : 464, className : "snikket.Client", methodName : "new"});
					return snikket_EventResult.EventUnhandled;
				}
	...
	onConnected(data) {
		...
		this.sendStanza(new snikket_Stanza("iq",{ type : "set", id : snikket_ID.short()}).tag("enable",{ xmlns : "urn:xmpp:carbons:2"}).up());
		let _gthis = this;
		...
		this.persistence.findServicesWithFeature(this.accountId(),"urn:xmpp:http:upload:0",function(services) {
			...
			tink_io_Source.chunked(new tink_io_js_BlobSource(source.name,source,0,chunkSize)).forEach(tink_streams_Handler.ofSafeSync(function(chunk) {
				...
				if(o._hx_index == 3) {
					...
					haxe_Log.trace("Error computing attachment hash",{ fileName : "snikket/Client.hx", lineNumber : 659, className : "snikket.Client", methodName : "prepareAttachment", customParams : [o]});
					callback(null);
				}
			});
		});
	...
	findAvailableChats(q,callback) {
			...
			discoGet.onFinished(function() {
				...
				if(resultCaps == null) {
					...
					if(err == null || (err != null ? err.name : null) == "service-unavailable" || (err != null ? err.name : null) == "feature-not-implemented") {
						results.push(new snikket_AvailableChat(jid.asString(),query,jid.asString(),new snikket_Caps("",[],[])));
					}
				} else {
					...
					results.push(new snikket_AvailableChat(jid.asString(),displayName,note,resultCaps));
				}
				callback(q,results);
			});
			_gthis.sendQuery(discoGet);
		...
		if(query.startsWith("xmpp:")) {
			checkAndAdd(snikket_JID.parse(HxOverrides.substr(query,5,null)));
		} else {
			let jid = snikket_JID.parse(query);
			if(jid.isValid()) {
				checkAndAdd(jid);
			}
		}
	...
	startChat(availableChat) {
		...
		if(availableChat.isChannel()) {
			let channel = new snikket_Channel(this,this.stream,this.persistence,availableChat.chatId,1,null,null,null,availableChat.caps);
		...
		if(availableChat.displayName != null) {
			chat.setDisplayName(availableChat.displayName);
		}
		this.persistence.storeChat(this.accountId(),chat);
	...
	getIceServers(callback) {
		...
		extDiscoGet.onFinished(function() {
			...
			while(_g < _g1.length) {
				...
				servers.push({ username : service.attr["username"], credential : service.attr["password"], urls : [service.attr["type"] + ":" + (host.indexOf(":") >= 0 ? "[" + host + "]" : host) + ":" + port + (isTurn ? "?transport=" + service.attr["transport"] : "")]});
			}
			callback(servers);
		});
		...
		rosterGet.onFinished(function() {
			...
			while(_g < _g1.length) {
				...
				if(item.fn != null && item.fn != "") {
					chat.setDisplayName(item.fn);
				}
				_gthis.persistence.storeChat(_gthis.accountId(),chat);
			}
			_gthis.trigger("chats/update",_gthis.chats);
		});
		...
		discoGet.onFinished(function() {
			...
			if(resultCaps == null) {
				...
				if(resultCaps.isChannel(jid)) {
					let chat = new snikket_Channel(_gthis,_gthis.stream,_gthis.persistence,jid,uiState,null,null,null,resultCaps);
					handleChat(chat);
		...
		sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 1:
					let message = m.message;
					promises.push(thenshim_Promise._new(function(resolve,reject) {
						_gthis.persistence.storeMessage(_gthis.accountId(),message,resolve);
					}));
					break;
				case 2:
					let update = m.update;
					promises.push(thenshim_Promise._new(function(resolve,reject) {
						_gthis.persistence.storeReaction(_gthis.accountId(),update,function(m) {
						});
					}));
					break;
				default:
	...
	pingAllChannels() {
		...
		while(_g < _g1.length) {
			...
			let channel = ((chat) instanceof snikket_Channel) ? chat : null;
			if(channel != null) {
				channel.selfPing((channel != null ? channel.disco : null) == null);
			}
		}
	}
}
...
Object.assign(snikket_Client.prototype, {
	...
	,sendAvailable: null
	,stream: null
	,chatMessageHandlers: null
	,chatStateHandlers: null
	,jid: null
	,chats: null
	,persistence: null
	,caps: null
...
class snikket_Date {
	static format(d) {
		let str = DateTools.format(d,"%Y-%m-%dT%H:%M:%S");
		let tzHour = d.getTimezoneOffset() / 60 | 0;
		let tzMinute = Math.abs(d.getTimezoneOffset()) % 60 | 0;
		return str + (tzHour < 0 ? "+" : "-") + StringTools.lpad(Std.string(Math.abs(tzHour)),"0",2) + ":" + StringTools.lpad(tzMinute == null ? "null" : "" + tzMinute,"0",2);
	}
}
...
class snikket_Hash {
	...
	toBase64() {
		return haxe_crypto_Base64.encode(haxe_io_Bytes.ofData(this.hash));
	}
	toBase64Url() {
...
var snikket_MessageStanza = $hxEnums["snikket.MessageStanza"] = { __ename__:true,__constructs__:null
	...
	,ReactionUpdateStanza: ($_=function(update) { return {_hx_index:2,update:update,__enum__:"snikket.MessageStanza",toString:$estr}; },$_._hx_name="ReactionUpdateStanza",$_.__params__ = ["update"],$_)
	,UnknownMessageStanza: ($_=function(stanza) { return {_hx_index:3,stanza:stanza,__enum__:"snikket.MessageStanza",toString:$estr}; },$_._hx_name="UnknownMessageStanza",$_.__params__ = ["stanza"],$_)
};
snikket_MessageStanza.__constructs__ = [snikket_MessageStanza.ErrorMessageStanza,snikket_MessageStanza.ChatMessageStanza,snikket_MessageStanza.ReactionUpdateStanza,snikket_MessageStanza.UnknownMessageStanza];
class snikket_Message {
	...
	static fromStanza(stanza,localJid,inputTimestamp) {
		...
		let msg = new snikket_ChatMessage();
		let tmp = stanza.findText("{urn:xmpp:delay}delay@stamp");
		let tmp1 = tmp != null ? tmp : inputTimestamp;
		...
		msg.isGroupchat = stanza.attr["type"] == "groupchat";
		let tmp2;
		if(stanza.attr["type"] == "groupchat") {
			tmp2 = msg.from;
		} else {
			let tmp = msg.from;
			tmp2 = tmp != null ? tmp.asBare() : null;
		}
		msg.sender = tmp2;
		...
		if(msg.direction == 0 && from1 != null) {
			let k = stanza.attr["type"] == "groupchat" ? from1.asBare().asString() : from1.asString();
			replyTo.set(k,true);
		} else if(msg.to != null) {
		...
		if(addresses != null) {
			...
			while(_g < _g1.length) {
				...
				if(address.attr["type"] == "noreply") {
					...
					haxe_Log.trace("No support for addressing to non-jid",{ fileName : "snikket/Message.hx", lineNumber : 112, className : "snikket.Message", methodName : "fromStanza", customParams : [address]});
					return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.UnknownMessageStanza(stanza));
					...
					if(snikket_JID.parse(jid).domain == tmp1) {
						msg.sender = snikket_JID.parse(jid).asBare();
					}
				}
			}
		}
		...
		if(msg.direction == 0 && msgFrom != null && Lambda.find(msg.replyTo,function(r) {
			...
			haxe_Log.trace("Don't know what chat message without from in replyTo belongs in",{ fileName : "snikket/Message.hx", lineNumber : 139, className : "snikket.Message", methodName : "fromStanza", customParams : [stanza]});
			return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.UnknownMessageStanza(stanza));
		}
		...
		if(reactionsEl != null) {
			...
			if(reactionId != null) {
				let tmp = msg.chatId();
				let tmp1 = msg.senderId();
				let tmp2 = stanza.attr["id"];
				return new snikket_Message(tmp,tmp1,msg.threadId,snikket_MessageStanza.ReactionUpdateStanza(new snikket_ReactionUpdate(tmp2 != null ? tmp2 : snikket_ID.long(),stanza.attr["type"] == "groupchat" ? reactionId : null,stanza.attr["type"] != "groupchat" ? reactionId : null,msg.chatId(),timestamp,msg.senderId(),reactions)));
			}
		}
		let _g4 = 0;
		...
		if(msg.text == null && msg.attachments.length < 1) {
			return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.UnknownMessageStanza(stanza));
		}
		let _g8 = 0;
		...
		if(reply != null) {
			...
			if(replyToID != null) {
				let replyToMessage = new snikket_ChatMessage();
				replyToMessage.isGroupchat = msg.isGroupchat;
				replyToMessage.from = replyToJid == null ? null : snikket_JID.parse(replyToJid);
				if(msg.isGroupchat) {
					replyToMessage.set_serverId(replyToID);
				} else {
					replyToMessage.set_localId(replyToID);
				}
				msg.replyToMessage = replyToMessage;
			}
		}
		let replace = stanza.getChild("replace","urn:xmpp:message-correct:0");
		let tmp3 = replace != null ? replace.attr : null;
		let replaceId = tmp3 != null ? tmp3["id"] : null;
		if(replaceId != null) {
			msg.versions = [msg.clone()];
			msg["localId"] = replaceId;
		}
		return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.ChatMessageStanza(msg));
	}
}
...
class snikket_MessageSync {
	...
	fetchNext() {
		...
		let resultHandler = this.stream.on("message",function(event) {
			...
			let msg = snikket_Message.fromStanza(originalMessage,_gthis.client.jid,timestamp).parsed;
			if(msg._hx_index == 1) {
				let chatMessage = msg.message;
				chatMessage.set_serverId(result.attr["id"]);
				chatMessage.serverIdBy = _gthis.serviceJID;
...
class snikket_Notification {
	constructor(title,body,accountId,chatId,messageId,imageUri,lang,timestamp) {
		this.title = title;
		this.body = body;
	...
	static fromChatMessage(m) {
		...
		return new snikket_Notification("New Message",m.text,m.account(),m.chatId(),m.serverId,imageUri,m.lang,m.timestamp);
	}
	static fromThinStanza(stanza) {
		return new snikket_Notification("New Message","",snikket_JID.parse(stanza.attr["to"]).asBare().asString(),snikket_JID.parse(stanza.attr["from"]).asBare().asString(),stanza.getChildText("stanza-id","urn:xmpp:sid:0"),null,null,null);
	}
}
...
class snikket_Participant {
	constructor(displayName,photoUri,placeholderUri) {
		this.displayName = displayName;
...
class snikket_ReactionUpdate {
	constructor(updateId,serverId,localId,chatId,timestamp,senderId,reactions) {
...
class snikket_TextNode {
	...
	serialize() {
		return StringTools.replace(StringTools.replace(StringTools.replace(this.content,"&","&amp;"),"<","&lt;"),">","&gt;");
	}
	clone() {
...
class snikket_jingle_IncomingProposedSession {
	...
	terminate() {
		haxe_Log.trace("Tried to terminate before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 68, className : "snikket.jingle.IncomingProposedSession", methodName : "terminate", customParams : [this]});
	}
	contentAdd(_) {
		haxe_Log.trace("Got content-add before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 72, className : "snikket.jingle.IncomingProposedSession", methodName : "contentAdd", customParams : [this]});
	}
	contentAccept(_) {
		haxe_Log.trace("Got content-accept before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 76, className : "snikket.jingle.IncomingProposedSession", methodName : "contentAccept", customParams : [this]});
	}
	transportInfo(_) {
		haxe_Log.trace("Got transport-info before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 80, className : "snikket.jingle.IncomingProposedSession", methodName : "transportInfo", customParams : [this]});
	...
	accept() {
		...
		this.client.sendStanza(new snikket_Stanza("message",{ to : this.from.asString(), type : "chat"}).tag("proceed",{ xmlns : "urn:xmpp:jingle-message:0", id : this.get_sid()}).up().tag("store",{ xmlns : "urn:xmpp:hints"}));
	}
	initiate(stanza) {
...
class snikket_jingle_OutgoingProposedSession {
	...
	propose(audio,video) {
		this.audio = audio;
		this.video = video;
		let stanza = new snikket_Stanza("message",{ to : this.to.asString(), type : "chat"}).tag("propose",{ xmlns : "urn:xmpp:jingle-message:0", id : this.get_sid()});
		if(audio) {
		...
		this.client.sendStanza(stanza);
		this.client.trigger("call/ringing",{ chatId : this.to.asBare().asString()});
	}
	ring() {
		haxe_Log.trace("Tried to accept before initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 156, className : "snikket.jingle.OutgoingProposedSession", methodName : "ring", customParams : [this]});
	}
	hangup() {
		this.client.sendStanza(new snikket_Stanza("message",{ to : this.to.asString(), type : "chat"}).tag("retract",{ xmlns : "urn:xmpp:jingle-message:0", id : this.get_sid()}).up().tag("store",{ xmlns : "urn:xmpp:hints"}));
	...
	terminate() {
		haxe_Log.trace("Tried to terminate before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 174, className : "snikket.jingle.OutgoingProposedSession", methodName : "terminate", customParams : [this]});
	}
	contentAdd(_) {
		haxe_Log.trace("Got content-add before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 178, className : "snikket.jingle.OutgoingProposedSession", methodName : "contentAdd", customParams : [this]});
	}
	contentAccept(_) {
		haxe_Log.trace("Got content-accept before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 182, className : "snikket.jingle.OutgoingProposedSession", methodName : "contentAccept", customParams : [this]});
	}
	transportInfo(_) {
		haxe_Log.trace("Got transport-info before session-initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 186, className : "snikket.jingle.OutgoingProposedSession", methodName : "transportInfo", customParams : [this]});
		return Promise.resolve(null);
	}
	accept() {
		haxe_Log.trace("Tried to accept before initiate: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 191, className : "snikket.jingle.OutgoingProposedSession", methodName : "accept", customParams : [this]});
	}
	initiate(stanza) {
...
class snikket_jingle_InitiatedSession {
	...
	retract() {
		haxe_Log.trace("Tried to retract session in wrong state: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 280, className : "snikket.jingle.InitiatedSession", methodName : "retract", customParams : [this]});
	}
	accept() {
	...
	initiate(stanza) {
		haxe_Log.trace("Trying to initiate already initiated session: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 303, className : "snikket.jingle.InitiatedSession", methodName : "initiate"});
	...
	sendIceCandidate(candidate) {
		...
		let transportInfo = new snikket_jingle_TransportInfo(new snikket_jingle_Media(media.mid,media.media,media.connectionData,media.port,media.protocol,[snikket_jingle_Attribute.parse(candidate.candidate),new snikket_jingle_Attribute("ice-ufrag",candidate.usernameFragment),Lambda.find(media.attributes,function(attr) {
			return attr.key == "ice-pwd";
		})],media.formats),this.get_sid()).toStanza(this.initiator);
		transportInfo.attr["to"] = this.counterpart.asString();
		transportInfo.attr["id"] = snikket_ID.medium();
		this.client.sendStanza(transportInfo);
	}
	supplyMedia(streams) {
		let _gthis = this;
		this.setupPeerConnection(function() {
			...
			if(_gthis.afterMedia == null) {
				_gthis.onPeerConnection().catch(function(e) {
					haxe_Log.trace("supplyMedia error",{ fileName : "snikket/jingle/Session.hx", lineNumber : 464, className : "snikket.jingle.InitiatedSession", methodName : "supplyMedia", customParams : [e]});
					_gthis.pc.close();
				});
			} else {
				_gthis.afterMedia();
	...
	setupPeerConnection(callback) {
		...
		this.client.getIceServers(function(servers) {
			_gthis.pc = new RTCPeerConnection(null,{ iceServers : servers});
			...
			_gthis.pc.addEventListener("negotiationneeded",function(event) {
				haxe_Log.trace("renegotiate",{ fileName : "snikket/jingle/Session.hx", lineNumber : 483, className : "snikket.jingle.InitiatedSession", methodName : "setupPeerConnection", customParams : [event]});
			});
	...
	setupLocalDescription(type,filterMedia,filterOut,beforeSend) {
		...
		return this.pc.setLocalDescription(null).then(function(_) {
			if((type == "session-initiate" || type == "session-accept") && _gthis.caps.features.includes("urn:ietf:rfc:3264")) {
...
class snikket_persistence_Dummy {
	...
	storeMessage(accountId,message,callback) {
		callback(message);
	}
...
class snikket_queries_MAMQuery extends snikket_queries_GenericQuery {
	...
	getResult() {
		...
		if(this.result == null) {
			...
			this.result = { complete : fin.attr["complete"] == "true" || fin.attr["complete"] == "1", page : { first : rsmInfo.getChildText("first"), last : rsmInfo.getChildText("last")}};
		}
		return this.result;
	}
}
...
class snikket_streams_XmppJsStream extends snikket_GenericStream {
	...
	connectWithURI(uri) {
		haxe_Log.trace("Got connection URI: " + uri,{ fileName : "snikket/streams/XmppJsStream.hx", lineNumber : 158, className : "snikket.streams.XmppJsStream", methodName : "connectWithURI"});
		if(uri == null) {
		...
		let xmpp = new snikket_streams_XmppJsClient({ service : this.connectionURI, domain : this.jid.domain, resource : this.jid.resource, clientId : clientId, credentials : function(callback,mechanisms) {
			_gthis.clientId = ((mechanisms) instanceof Array) ? clientId : null;
			let mechs = ((mechanisms) instanceof Array) ? mechanisms : [{ name : mechanisms, canFast : false, canOther : true}];
			let tmp = Lambda.find(mechs,function(m) {
				return m.canOther;
			});
			let mech = tmp != null ? tmp.name : null;
			_gthis.trigger("auth/password-needed",{ mechanisms : mechs});
			return waitForCreds.then(function(creds) {
				let tmp = creds.mechanism;
				return callback(creds,tmp != null ? tmp : mech);
			});
		}});
		let tmp = xmpp.sasl2;
		snikket_streams_XmppJsScramSha1(tmp != null ? tmp : xmpp.sasl);
		...
		if(this.initialSM != null) {
			...
			let f = snikket_streams_XmppJsLtx.parse;
			let result = new Array(_this.length);
			let _g = 0;
			let _g1 = _this.length;
			while(_g < _g1) {
		...
		xmpp.middleware.use(function(data) {
			...
			if(data.stanza.attrs.xmlns == "urn:xmpp:sm:3") {
				return;
			}
			if(xmpp.status == "online" && _gthis.state.can("connection-success")) {
				_gthis.resumed = xmpp.streamManagement.enabled && xmpp.streamManagement.id != null && xmpp.streamManagement.id != "";
				if(xmpp.jid == null) {
					xmpp.jid = _gthis.jid;
				} else {
					_gthis.jid = xmpp.jid;
		...
		xmpp.on("stream-management/ack",function(stanza) {
			if(stanza.name == "message" && stanza.attrs.id != null) {
				_gthis.trigger("sm/ack",{ id : stanza.attrs.id});
			}
			_gthis.triggerSMupdate();
		});
		xmpp.on("stream-management/fail",function(stanza) {
			if(stanza.name == "message" && stanza.attrs.id != null) {
				_gthis.trigger("sm/fail",{ id : stanza.attrs.id});
			}
			_gthis.triggerSMupdate();
		});
		xmpp.on("fast-token",function(tokenEl) {
			_gthis.trigger("fast-token",tokenEl.attrs);
		});
		...
		xmpp.start().catch(function(err) {
			...
			if((xmppError != null ? xmppError.name : null) == "SASLError") {
				...
				haxe_Log.trace(err,{ fileName : "snikket/streams/XmppJsStream.hx", lineNumber : 277, className : "snikket.streams.XmppJsStream", methodName : "connectWithURI"});
			}
		});
	}
	connect(jid,sm) {
	...
	triggerSMupdate() {
		if(!this.client.streamManagement.enabled || !this.client.streamManagement.allowResume) {
			return;
		}
...
snikket_Autolink.GOOD_IRI_CHAR = "a-zA-Z0-9 -퟿豈-﷏ﷰ-￯";
...
snikket_persistence_Dummy.__meta__ = { fields : { lastId : { 'HaxeCBridge.noemit' : null}, storeChat : { 'HaxeCBridge.noemit' : null}, getChats : { 'HaxeCBridge.noemit' : null}, storeMessage : { 'HaxeCBridge.noemit' : null}, getMessagesBefore : { 'HaxeCBridge.noemit' : null}, getMessagesAfter : { 'HaxeCBridge.noemit' : null}, getMessagesAround : { 'HaxeCBridge.noemit' : null}, getChatsUnreadDetails : { 'HaxeCBridge.noemit' : null}, storeReaction : { 'HaxeCBridge.noemit' : null}, updateMessageStatus : { 'HaxeCBridge.noemit' : null}, getMediaUri : { 'HaxeCBridge.noemit' : null}, hasMedia : { 'HaxeCBridge.noemit' : null}, storeMedia : { 'HaxeCBridge.noemit' : null}, storeCaps : { 'HaxeCBridge.noemit' : null}, getCaps : { 'HaxeCBridge.noemit' : null}, storeLogin : { 'HaxeCBridge.noemit' : null}, getLogin : { 'HaxeCBridge.noemit' : null}, removeAccount : { 'HaxeCBridge.noemit' : null}, storeStreamManagement : { 'HaxeCBridge.noemit' : null}, getStreamManagement : { 'HaxeCBridge.noemit' : null}, storeService : { 'HaxeCBridge.noemit' : null}, findServicesWithFeature : { 'HaxeCBridge.noemit' : null}}};
snikket_queries_GenericQuery._hx_skip_constructor = false;
