			...
			while(_g < _g1.length) {
				...
				case 1:
					...
					promises.push(thenshim_Promise._new(function(resolve,reject) {
						_gthis.client.storeMessage(chatMessage,resolve);
					}));
					break;
				case 2:
					let update = m.update;
...
class snikket_DirectChat extends snikket_Chat {
	...
	correctMessage(localId,message) {
		...
		let _gthis = this;
		this.client.storeMessage(message,function(corrected) {
	...
	sendMessage(message) {
		...
		case 1:
			...
			this.client.storeMessage(message,function(stored) {
				let _g = 0;
				let _g1 = message.recipients;
				while(_g < _g1.length) {
					let recipient = _g1[_g];
		...
		case 2:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 708, className : "snikket.DirectChat", methodName : "sendMessage", customParams : [fromStanza]});
	...
	removeReaction(m,reaction) {
		...
		let update = new snikket_ReactionUpdate(snikket_ID.long(),null,null,m.localId,m.chatId(),snikket_Date.format(new Date()),this.client.accountId(),reactions);
		let _gthis = this;
		this.persistence.storeReaction(this.client.accountId(),update,function(stored) {
...
class snikket_Channel extends snikket_Chat {
		...
		sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 1:
					...
					promises.push(thenshim_Promise._new(function(resolve,reject) {
						_gthis.client.storeMessage(message,resolve);
					}));
	...
	sendMessage(message) {
		...
		case 2:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 1126, className : "snikket.Channel", methodName : "sendMessage", customParams : [fromStanza]});
			throw haxe_Exception.thrown("Trying to send invalid message.");
		}
	}
	removeReaction(m,reaction) {
		...
		let update = new snikket_ReactionUpdate(snikket_ID.long(),m.serverId,m.chatId(),null,m.chatId(),snikket_Date.format(new Date()),this.client.accountId(),reactions);
		let _gthis = this;
		this.persistence.storeReaction(this.client.accountId(),update,function(stored) {
			let stanza = update.asStanza();
...
class snikket_Client extends snikket_EventEmitter {
	...
	_hx_constructor(address,persistence) {
			...
			if(jmiPro != null && jmiPro.attr["id"] != null) {
				...
				if(session != null) {
					try {
						...
						haxe_Log.trace("JMI proceed failed",{ fileName : "snikket/Client.hx", lineNumber : 220, className : "snikket.Client", methodName : "new", customParams : [e]});
					}
				}
			}
		...
		this.stream.onIq(snikket_IqRequestType.Set,"jingle","urn:xmpp:jingle:1",function(stanza) {
			...
			if(jingle.attr["action"] == "session-initiate") {
				if(session != null) {
					try {
						...
						let e = haxe_Exception.caught(_g);
						haxe_Log.trace("Bad session-inititate",{ fileName : "snikket/Client.hx", lineNumber : 322, className : "snikket.Client", methodName : "new", customParams : [e]});
						chat.jingleSessions.delete(session.get_sid());
					}
				} else {
			...
			if(session != null && jingle.attr["action"] == "session-accept") {
				try {
					...
					let e = haxe_Exception.caught(_g);
					haxe_Log.trace("session-accept failed",{ fileName : "snikket/Client.hx", lineNumber : 337, className : "snikket.Client", methodName : "new", customParams : [e]});
				}
			}
		...
		this.stream.on("presence",function(event) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == null) {
				...
				let chat = _gthis.getChat(from.asBare().asString());
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "snikket/Client.hx", lineNumber : 398, className : "snikket.Client", methodName : "new"});
					return snikket_EventResult.EventUnhandled;
				}
				if(c == null) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == "unavailable") {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "snikket/Client.hx", lineNumber : 468, className : "snikket.Client", methodName : "new"});
					return snikket_EventResult.EventUnhandled;
				}
		...
		this.persistence.findServicesWithFeature(this.accountId(),"urn:xmpp:http:upload:0",function(services) {
			...
			tink_io_Source.chunked(new tink_io_js_BlobSource(source.name,source,0,chunkSize)).forEach(tink_streams_Handler.ofSafeSync(function(chunk) {
				...
				if(o._hx_index == 3) {
					...
					haxe_Log.trace("Error computing attachment hash",{ fileName : "snikket/Client.hx", lineNumber : 663, className : "snikket.Client", methodName : "prepareAttachment", customParams : [o]});
					callback(null);
				}
			});
		});
	...
	storeMessage(message,callback) {
		let tmp = callback;
		this.persistence.storeMessage(this.accountId(),message,tmp != null ? tmp : function(_) {
		});
	}
	sendQuery(query) {
...
class snikket_Message {
	...
	static fromStanza(stanza,localJid,inputTimestamp) {
		...
		if(addresses != null) {
			...
			while(_g < _g1.length) {
				...
				if(address.attr["type"] == "noreply") {
					...
					haxe_Log.trace("No support for addressing to non-jid",{ fileName : "snikket/Message.hx", lineNumber : 126, className : "snikket.Message", methodName : "fromStanza", customParams : [address]});
		...
		if(msg.direction == 0 && msgFrom != null && Lambda.find(msg.replyTo,function(r) {
			...
			haxe_Log.trace("Don't know what chat message without from in replyTo belongs in",{ fileName : "snikket/Message.hx", lineNumber : 153, className : "snikket.Message", methodName : "fromStanza", customParams : [stanza]});
			return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.UnknownMessageStanza(stanza));
		}
		...
		if(reactionsEl != null) {
			...
			if(reactionId != null) {
				...
				return new snikket_Message(tmp,tmp1,msg1,snikket_MessageStanza.ReactionUpdateStanza(new snikket_ReactionUpdate(tmp3,isGroupchat ? reactionId : null,tmp4,isGroupchat ? null : reactionId,msg.chatId(),timestamp,msg.senderId(),reactions)));
			}
		}
		let _g4 = 0;
		...
		if(reply != null) {
			...
			if(text != null && snikket_EmojiUtil.isOnlyEmoji(StringTools.trim(text))) {
				...
				let tmp4 = isGroupchat ? msg.chatId() : null;
				let tmp5 = msg.chatId();
				return new snikket_Message(tmp,tmp1,msg1,snikket_MessageStanza.ReactionUpdateStanza(new snikket_ReactionUpdate(tmp3,isGroupchat ? replyToID : null,tmp4,isGroupchat ? null : replyToID,tmp5,timestamp,msg.senderId(),[StringTools.trim(text)],true)));
			}
			if(html != null) {
				...
				if(body != null) {
					...
					if(els.length == 1 && els[0].name == "img") {
						...
						if(hash != null) {
							...
							let tmp4 = isGroupchat ? msg.chatId() : null;
							let tmp5 = msg.chatId();
							return new snikket_Message(tmp,tmp1,msg1,snikket_MessageStanza.ReactionUpdateStanza(new snikket_ReactionUpdate(tmp3,isGroupchat ? replyToID : null,tmp4,isGroupchat ? null : replyToID,tmp5,timestamp,msg.senderId(),[hash.serializeUri()],true)));
						}
					}
				}
			}
...
class snikket_ReactionUpdate {
	constructor(updateId,serverId,serverIdBy,localId,chatId,timestamp,senderId,reactions,append) {
		if(append == null) {
			append = false;
		}
	...
	getReactions(existingReactions) {
		if(this.append) {
			...
			return Lambda.array({ iterator : function() {
				return new js_lib_HaxeIterator(set.keys());
			}});
		} else {
	...
	inlineHashReferences() {
		...
		while(_g < _g1.length) {
			let r = _g1[_g];
			++_g;
			let hash = snikket_Hash.fromUri(r);
			if(hash != null) {
				hashes.push(hash);
			}
		}
	...
	asStanza() {
		if(this.append) {
			throw haxe_Exception.thrown("Cannot make a reaction XEP stanza for an append");
		}
...
class snikket_jingle_IncomingProposedSession {
	...
	ring() {
		...
		let _gthis = this;
		this.client.storeMessage(msg,function(stored) {
			_gthis.client.notifyMessageHandlers(stored);
		});
...
class snikket_jingle_InitiatedSession {
	...
	retract() {
		haxe_Log.trace("Tried to retract session in wrong state: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 329, className : "snikket.jingle.InitiatedSession", methodName : "retract", customParams : [this]});
	}
	accept() {
	...
	initiate(stanza) {
		haxe_Log.trace("Trying to initiate already initiated session: " + this.get_sid(),{ fileName : "snikket/jingle/Session.hx", lineNumber : 352, className : "snikket.jingle.InitiatedSession", methodName : "initiate"});
		...
		this.setupPeerConnection(function() {
			...
			if(_gthis.afterMedia == null) {
				_gthis.onPeerConnection().catch(function(e) {
					haxe_Log.trace("supplyMedia error",{ fileName : "snikket/jingle/Session.hx", lineNumber : 525, className : "snikket.jingle.InitiatedSession", methodName : "supplyMedia", customParams : [e]});
					_gthis.pc.close();
				});
			} else {
				_gthis.afterMedia();
	...
	setupPeerConnection(callback) {
		...
		this.client.getIceServers(function(servers) {
			...
			_gthis.pc.addEventListener("negotiationneeded",function(event) {
				haxe_Log.trace("renegotiate",{ fileName : "snikket/jingle/Session.hx", lineNumber : 544, className : "snikket.jingle.InitiatedSession", methodName : "setupPeerConnection", customParams : [event]});
			});
			...
			_gthis.pc.addEventListener("connectionstatechange",function(event) {
				if(_gthis.pc.connectionState == "closed" || _gthis.pc.connectionState == "failed") {
					_gthis.terminate();
				}
			});
			callback();
...
function snikket_jingle_Session_mkCallMessage(to,from,event) {
	...
	m.set_timestamp(snikket_Date.format(new Date()));
	m.payloads.push(event);
	m.set_localId(snikket_ID.long());
...
class snikket_streams_XmppJsStream extends snikket_GenericStream {
	...
	connectWithURI(uri) {
		haxe_Log.trace("Got connection URI: " + uri,{ fileName : "snikket/streams/XmppJsStream.hx", lineNumber : 154, className : "snikket.streams.XmppJsStream", methodName : "connectWithURI"});
		if(uri == null) {
		...
		xmpp.start().catch(function(err) {
			...
			if((xmppError != null ? xmppError.name : null) == "SASLError") {
				...
				haxe_Log.trace(err,{ fileName : "snikket/streams/XmppJsStream.hx", lineNumber : 273, className : "snikket.streams.XmppJsStream", methodName : "connectWithURI"});
			}
		});
	}
	connect(jid,sm) {
	...
	triggerSMupdate() {
		if(this.client == null || !this.client.streamManagement.enabled || !this.client.streamManagement.allowResume) {
			return;
		}
