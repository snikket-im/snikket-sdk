...
class snikket_Chat {
	...
	addReaction(m,reaction) {
		let toSend = m.reply();
		toSend.set_localId(snikket_ID.long());
...
class snikket_DirectChat extends snikket_Chat {
	...
	correctMessage(localId,message) {
		...
		this.client.storeMessage(message,function(corrected) {
			...
			_gthis.client.notifyMessageHandlers(corrected,1);
		});
	}
	sendMessage(message) {
		if(this.typingTimer != null) {
		...
		case 1:
			...
			this.client.storeMessage(message,function(stored) {
				...
				_gthis.client.trigger("chats/update",[_gthis]);
				_gthis.client.notifyMessageHandlers(stored,stored.versions.length > 1 ? 1 : 0);
			});
			break;
		case 2:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 781, className : "snikket.DirectChat", methodName : "sendMessage", customParams : [fromStanza]});
	...
	removeReaction(m,reaction) {
		if(((reaction) instanceof snikket_CustomEmojiReaction)) {
			...
			let correct = m.reply();
			correct.set_localId(snikket_ID.long());
			correct.setHtml("");
			correct.text = null;
...
class snikket_Channel extends snikket_Chat {
		...
		(refresh ? $bind(this,this.refreshDisco) : function(cb) {
			...
			_gthis.stream.sendIq(new snikket_Stanza("iq",{ type : "get", to : _gthis.getFullJid().asString()}).tag("ping",{ xmlns : "urn:xmpp:ping"}).up(),function(response) {
				if(response.attr["type"] == "error") {
					...
					_gthis.inSync = false;
					_gthis.persistence.lastId(_gthis.client.accountId(),_gthis.chatId,$bind(_gthis,_gthis.doSync));
					_gthis.checkRename();
				}
			});
		});
	...
	doSync(lastId) {
		...
		this.sync.onError(function(stanza) {
			...
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC failed",{ fileName : "snikket/Chat.hx", lineNumber : 1077, className : "snikket.Channel", methodName : "doSync", customParams : [_gthis.chatId,stanza]});
			}
		});
		this.sync.fetchNext();
	}
	...
	sendMessage(message) {
		...
		case 2:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "snikket/Chat.hx", lineNumber : 1269, className : "snikket.Channel", methodName : "sendMessage", customParams : [fromStanza]});
			throw haxe_Exception.thrown("Trying to send invalid message.");
		}
	}
	removeReaction(m,reaction) {
...
class snikket_Client extends snikket_EventEmitter {
	...
	_hx_constructor(address,persistence) {
			...
			if(jmiPro != null && jmiPro.attr["id"] != null) {
				...
				if(session != null) {
					try {
						...
						haxe_Log.trace("JMI proceed failed",{ fileName : "snikket/Client.hx", lineNumber : 236, className : "snikket.Client", methodName : "new", customParams : [e]});
					}
				}
			}
		...
		this.stream.onIq(snikket_IqRequestType.Set,"jingle","urn:xmpp:jingle:1",function(stanza) {
			...
			if(jingle.attr["action"] == "session-initiate") {
				if(session != null) {
					try {
						...
						let e = haxe_Exception.caught(_g);
						haxe_Log.trace("Bad session-inititate",{ fileName : "snikket/Client.hx", lineNumber : 343, className : "snikket.Client", methodName : "new", customParams : [e]});
						chat.jingleSessions.delete(session.get_sid());
					}
				} else {
			...
			if(session != null && jingle.attr["action"] == "session-accept") {
				try {
					...
					let e = haxe_Exception.caught(_g);
					haxe_Log.trace("session-accept failed",{ fileName : "snikket/Client.hx", lineNumber : 358, className : "snikket.Client", methodName : "new", customParams : [e]});
				}
			}
		...
		this.stream.on("presence",function(event) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == null) {
				...
				let chat = _gthis.getChat(from.asBare().asString());
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "snikket/Client.hx", lineNumber : 460, className : "snikket.Client", methodName : "new"});
					return snikket_EventResult.EventUnhandled;
				}
				if(c == null) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == "unavailable") {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "snikket/Client.hx", lineNumber : 530, className : "snikket.Client", methodName : "new"});
					return snikket_EventResult.EventUnhandled;
				}
	...
	onConnected(data) {
		...
		this.rosterGet();
		haxe_Log.trace("SYNC: bookmarks",{ fileName : "snikket/Client.hx", lineNumber : 684, className : "snikket.Client", methodName : "onConnected"});
		this.bookmarksGet(function() {
			haxe_Log.trace("SYNC: MAM",{ fileName : "snikket/Client.hx", lineNumber : 686, className : "snikket.Client", methodName : "onConnected"});
			_gthis.sync(function(syncFinished) {
				if(!syncFinished) {
					haxe_Log.trace("SYNC: failed",{ fileName : "snikket/Client.hx", lineNumber : 689, className : "snikket.Client", methodName : "onConnected"});
					_gthis.inSync = false;
					_gthis.stream.disconnect();
					return;
				}
				haxe_Log.trace("SYNC: details",{ fileName : "snikket/Client.hx", lineNumber : 696, className : "snikket.Client", methodName : "onConnected"});
				_gthis.inSync = true;
				_gthis.persistence.getChatsUnreadDetails(_gthis.accountId(),_gthis.chats,function(details) {
					let _g = 0;
					while(_g < details.length) {
					...
					haxe_Log.trace("SYNC: done",{ fileName : "snikket/Client.hx", lineNumber : 716, className : "snikket.Client", methodName : "onConnected"});
				});
			});
		});
		...
		this.persistence.findServicesWithFeature(this.accountId(),"urn:xmpp:http:upload:0",function(services) {
			...
			tink_io_Source.chunked(new tink_io_js_BlobSource(source.name,source,0,chunkSize)).forEach(tink_streams_Handler.ofSafeSync(function(chunk) {
				...
				if(o._hx_index == 3) {
					...
					haxe_Log.trace("Error computing attachment hash",{ fileName : "snikket/Client.hx", lineNumber : 739, className : "snikket.Client", methodName : "prepareAttachment", customParams : [o]});
					callback(null);
				}
			});
		});
		...
		while(_g < _g1.length) {
			...
			if(chat.chatId != jid.asBare().asString()) {
				if(chat.chatId.includes(query.toLowerCase()) || chat.getDisplayName().toLowerCase().includes(query.toLowerCase())) {
					...
					results.push(new snikket_AvailableChat(chat.chatId,chat.getDisplayName(),chat.chatId,channel == null ? new snikket_Caps("",[],[]) : channel.disco));
				}
			}
			if(chat.isTrusted()) {
	...
	startChat(availableChat) {
		...
		if(existingChat != null) {
			...
			if(channel == null && availableChat.isChannel() || channel != null && !availableChat.isChannel()) {
				...
				this.persistence.storeChat(this.accountId(),existingChat);
	...
	doSync(callback,lastId) {
		...
		sync.onMessages(function(messageList) {
			...
			haxe_Log.trace("SYNC: MAM page wait for writes",{ fileName : "snikket/Client.hx", lineNumber : 1403, className : "snikket.Client", methodName : "doSync"});
			thenshim_Promise.then(thenshim_PromiseTools.all(promises),function(_) {
				...
				haxe_Log.trace("SYNC: error",{ fileName : "snikket/Client.hx", lineNumber : 1415, className : "snikket.Client", methodName : "doSync", customParams : [e == null ? "null" : Std.string(e)]});
				callback(false);
			});
		});
		sync.onError(function(stanza) {
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC: error",{ fileName : "snikket/Client.hx", lineNumber : 1424, className : "snikket.Client", methodName : "doSync", customParams : [stanza]});
				if(callback != null) {
					callback(false);
				}
			}
...
class snikket_Message {
	...
	static fromStanza(stanza,localJid,inputTimestamp) {
		...
		if(addresses != null) {
			...
			while(_g < _g1.length) {
				...
				if(address.attr["type"] == "noreply") {
					...
					haxe_Log.trace("No support for addressing to non-jid",{ fileName : "snikket/Message.hx", lineNumber : 127, className : "snikket.Message", methodName : "fromStanza", customParams : [address]});
		...
		if(msg.direction == 0 && msgFrom != null && Lambda.find(msg.replyTo,function(r) {
			...
			haxe_Log.trace("Don't know what chat message without from in replyTo belongs in",{ fileName : "snikket/Message.hx", lineNumber : 154, className : "snikket.Message", methodName : "fromStanza", customParams : [stanza]});
			return new snikket_Message(msg.chatId(),msg.senderId(),msg.threadId,snikket_MessageStanza.UnknownMessageStanza(stanza));
		}
		...
		if(reactionsEl != null) {
			...
			if(reactionId != null) {
				...
				let tmp7 = msg.chatId();
				let tmp8 = msg.senderId();
				let result = new Array(reactions.length);
				let _g = 0;
				let _g1 = reactions.length;
				while(_g < _g1) {
					let i = _g++;
					result[i] = new snikket_Reaction(msg.senderId(),timestamp,reactions[i],msg.localId);
				}
		...
		if(reply != null) {
			...
			if(text != null && snikket_EmojiUtil.isOnlyEmoji(StringTools.trim(text))) {
				...
				let tmp5 = msg.chatId();
				let tmp6 = msg.senderId();
				return new snikket_Message(tmp,tmp1,msg1,snikket_MessageStanza.ReactionUpdateStanza(new snikket_ReactionUpdate(tmp3,isGroupchat ? replyToID : null,tmp4,isGroupchat ? null : replyToID,tmp5,tmp6,timestamp,[new snikket_Reaction(msg.senderId(),timestamp,StringTools.trim(text),msg.localId)],1)));
			}
			if(html != null) {
...
class snikket_jingle_IncomingProposedSession {
	...
	ring() {
		...
		this.client.storeMessage(msg,function(stored) {
			_gthis.client.notifyMessageHandlers(stored,1);
		});
...
class snikket_jingle_OutgoingProposedSession {
	...
	propose(audio,video) {
		...
		this.client.storeMessage(msg,function(stored) {
			...
			_gthis.client.sendStanza(stanza);
			_gthis.client.notifyMessageHandlers(stored,0);
