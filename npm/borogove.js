...
class borogove_Autolink {
	...
	static match(s,start,pattern,addHttps) {
		if(pattern.matchSub(s,start)) {
			...
			return { span : borogove_Node.Element(new borogove_Stanza("a",{ href : uri}).text(link)), start : pos.pos, end : pos.pos + pos.len};
		} else {
...
class borogove_Caps {
	...
	computeVer() {
		...
		return borogove_Hash.sha1(haxe_io_Bytes.ofString(s));
	}
	verRaw() {
		if(this._ver == null) {
...
class borogove_Chat {
		...
		return thenshim_Promise._new(function(resolve,reject) {
			sync.onMessages(function(messageList) {
				...
				while(_g < _g1.length) {
					...
					case 3:
						let update = m.update;
						_gthis.persistence.storeReaction(_gthis.client.accountId(),update);
						break;
					default:
					}
				}
		...
		if(this.displayName == this.chatId) {
			...
			if(participants.length > 2 && participants.length < 20) {
				...
				while(_g < _g1) {
					...
					let p = id == _gthis.chatId ? null : _gthis.getParticipantDetails(id);
	...
	setCaps(resource,caps) {
		...
		if(presence != null) {
			...
			this.setPresence(resource,new borogove_Presence(caps,null));
		}
	}
	...
	markReadUpToId(upTo,upToBy,callback) {
		...
		this.persistence.storeChats(this.client.accountId(),[this]);
		let _gthis = this;
		thenshim_Promise.then(this.persistence.getMessagesBefore(this.client.accountId(),this.chatId,null,null),function(messages) {
	...
	markReadUpToMessage(message,callback) {
		...
		if(this.readUpTo() == null) {
			this.markReadUpToId(message.serverId,message.serverIdBy,callback);
			return;
		}
		let _gthis = this;
		thenshim_Promise.then(this.persistence.getMessage(this.client.accountId(),this.chatId,this.readUpTo(),null),function(readMessage) {
...
class borogove_DirectChat extends borogove_Chat {
	...
	sendMessage(message) {
		...
		case 3:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "borogove/Chat.hx", lineNumber : 928, className : "borogove.DirectChat", methodName : "sendMessage", customParams : [fromStanza]});
		...
		this.markReadUpToMessage(message,function() {
			if(message.isIncoming() && message.localId != null) {
				...
				while(_g < _g1.length) {
					let recipient = _g1[_g];
					++_g;
					let stanza = new borogove_Stanza("message",{ to : recipient, id : borogove_ID.long()}).tag("displayed",{ xmlns : "urn:xmpp:chat-markers:0", id : message.localId}).up();
...
class borogove_Channel extends borogove_Chat {
		...
		(refresh ? $bind(this,this.refreshDisco) : function(cb) {
			...
			_gthis.stream.sendIq(new borogove_Stanza("iq",{ type : "get", to : _gthis.getFullJid().asString()}).tag("ping",{ xmlns : "urn:xmpp:ping"}).up(),function(response) {
				if(response.attr["type"] == "error") {
					...
					haxe_Log.trace("SYNC: self-ping fail, join",{ fileName : "borogove/Chat.hx", lineNumber : 1093, className : "borogove.Channel", methodName : "selfPing", customParams : [_gthis.chatId]});
					_gthis.join();
				} else {
					_gthis.selfPingSuccess();
				}
	...
	doSync(lastId) {
		...
		this.sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 3:
					let update = m.update;
					promises.push(thenshim_Promise.then(_gthis.persistence.storeReaction(_gthis.client.accountId(),update),function(_) {
						return null;
					}));
					break;
				default:
		...
		this.sync.onError(function(stanza) {
			...
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC failed",{ fileName : "borogove/Chat.hx", lineNumber : 1257, className : "borogove.Channel", methodName : "doSync", customParams : [_gthis.chatId,stanza]});
			}
		});
		this.sync.fetchNext();
	}
	...
	refreshDisco(callback) {
		...
		discoGet.onFinished(function() {
			if(discoGet.getResult() != null) {
				let setupNotifications = _gthis.disco == null && _gthis.notificationSettings == null;
				_gthis.disco = discoGet.getResult();
				if(setupNotifications && !_gthis.isPrivate()) {
					_gthis.notificationSettings = { mention : true, reply : false};
				}
	...
	getParticipantDetails(participantId) {
		if(participantId == this.getFullJid().asString()) {
			...
			let tmp = nick;
			return new borogove_Participant(tmp != null ? tmp : "",null,placeholderUri,false);
		}
	}
	getMessagesBefore(beforeId,beforeTime) {
		let _gthis = this;
	...
	sendMessage(message) {
		...
		case 3:
			...
			break;
		default:
			haxe_Log.trace("Invalid message",{ fileName : "borogove/Chat.hx", lineNumber : 1456, className : "borogove.Channel", methodName : "sendMessage", customParams : [fromStanza]});
			throw haxe_Exception.thrown("Trying to send invalid message.");
		}
	}
	removeReaction(m,reaction) {
...
class borogove_ChatMessage {
	...
	html() {
		...
		let tmp1 = this.text;
		let codepoints = borogove_StringUtil.codepointArray(tmp1 != null ? tmp1 : "");
		let _gthis = this;
		let _g = [];
		let _g1 = 0;
	...
	asStanza() {
		...
		if(this.versions.length > 0 && this.versions[0].localId != null) {
			stanza.tag("replace",{ xmlns : "urn:xmpp:message-correct:0", id : this.versions[0].localId}).up();
		}
		if(this.threadId != null) {
		...
		while(_g < _g1.length) {
			...
			if(attachment.uris.length > 0) {
				...
				let codepoints = borogove_StringUtil.codepointArray(body);
		...
		while(_g2 < _g3.length) {
			let payload = _g3[_g2];
			++_g2;
			stanza.addDirectChild(borogove_Node.Element(payload));
		}
		return stanza;
	}
...
class borogove_ChatMessageBuilder {
	...
	attachSims(sims) {
		...
		let uris = _g4;
		if(uris.length > 0) {
			this.attachments.push(new borogove_ChatAttachment(name,mime,size == null ? null : Std.parseInt(size),uris,hashes));
		}
	}
	...
	build() {
		...
		return new borogove_ChatMessage({ localId : this.localId, serverId : this.serverId, serverIdBy : this.serverIdBy, type : this.type, syncPoint : this.syncPoint, replyId : this.replyId, timestamp : tmp1 != null ? tmp1 : borogove_Date.format(new Date()), to : to, from : from, senderId : this.get_senderId(), recipients : this.recipients, replyTo : this.replyTo, replyToMessage : this.replyToMessage, threadId : this.threadId, attachments : this.attachments, reactions : this.reactions, text : this.text, lang : this.lang, direction : this.direction, status : this.status, versions : this.versions, payloads : this.payloads, encryption : this.encryption, stanza : this.stanza});
	}
...
class borogove_EventEmitter {
	...
	on(eventName,callback) {
		...
		let newHandler = new borogove_EventHandler(handlers,callback);
	...
	trigger(eventName,eventData) {
		...
		if(handlers == null || handlers.length == 0) {
			haxe_Log.trace("no event handlers for " + eventName,{ fileName : "borogove/EventEmitter.hx", lineNumber : 29, className : "borogove.EventEmitter", methodName : "trigger"});
			return borogove_EventResult.EventUnhandled;
		}
		haxe_Log.trace("firing event: " + eventName,{ fileName : "borogove/EventEmitter.hx", lineNumber : 32, className : "borogove.EventEmitter", methodName : "trigger"});
		let handled = false;
		let _g = 0;
...
class borogove_Client extends borogove_EventEmitter {
	...
	_hx_constructor(address,persistence) {
		...
		this.stream.onIq(borogove_IqRequestType.Set,"jingle","urn:xmpp:jingle:1",function(stanza) {
			...
			if(jingle.attr["action"] == "session-initiate") {
				if(session != null) {
					try {
						...
						haxe_Log.trace("Bad session-inititate",{ fileName : "borogove/Client.hx", lineNumber : 210, className : "borogove.Client", methodName : "new", customParams : [e]});
			...
			if(session != null && jingle.attr["action"] == "session-accept") {
				try {
					...
					let e = haxe_Exception.caught(_g);
					haxe_Log.trace("session-accept failed",{ fileName : "borogove/Client.hx", lineNumber : 225, className : "borogove.Client", methodName : "new", customParams : [e]});
				}
			}
		...
		this.stream.on("presence",function(event) {
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == null) {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "borogove/Client.hx", lineNumber : 328, className : "borogove.Client", methodName : "new"});
					return borogove_EventResult.EventUnhandled;
				}
				if(c == null) {
					chat.setPresence(borogove_JID.parse(stanza.attr["from"]).resource,new borogove_Presence(null,mucUser));
					...
					let handleCaps = function(caps) {
						chat.setPresence(borogove_JID.parse(stanza.attr["from"]).resource,new borogove_Presence(caps,mucUser));
				...
				if(from.isBare()) {
					...
					if(avatarSha1Hex != null) {
						...
						persistence.storeChats(_gthis.accountId(),[chat]);
						thenshim_Promise.then(persistence.hasMedia("sha-1",avatarSha1),function(has) {
							if(has) {
								if(chat.livePresence()) {
									_gthis.trigger("chats/update",[chat]);
								}
			...
			if(stanza.attr["from"] != null && stanza.attr["type"] == "unavailable") {
				...
				if(chat == null) {
					haxe_Log.trace("Presence for unknown JID: " + stanza.attr["from"],{ fileName : "borogove/Client.hx", lineNumber : 397, className : "borogove.Client", methodName : "new"});
					return borogove_EventResult.EventUnhandled;
				}
	...
	processLiveMessage(stanza,fwd,encryptionInfo) {
		...
		case 1:
			...
			while(_g1 < _g2.length) {
				let hash = _g2[_g1];
				++_g1;
				this.fetchMediaByHash([hash],[chatMessage.from]);
			}
			let chat = this.getChat(chatMessage.chatId());
			if(chat != null) {
				...
				if(chatMessage.serverId == null) {
					...
					thenshim_Promise.then(this.storeMessages([chatMessage]),function(stored) {
						updateChat(stored[0]);
					});
				}
			}
		...
		case 2:
			...
			thenshim_Promise.then(this.moderateMessage(action),function(stored) {
				if(stored != null) {
		...
		case 3:
			...
			while(_g3 < _g4.length) {
				let hash = _g4[_g3];
				++_g3;
				this.fetchMediaByHash([hash],[from]);
			}
			thenshim_Promise.then(this.persistence.storeReaction(this.accountId(),update),function(stored) {
				if(stored != null) {
			...
			break;
		default:
			haxe_Log.trace("Ignoring non-chat message: " + stanza.toString(),{ fileName : "borogove/Client.hx", lineNumber : 457, className : "borogove.Client", methodName : "processLiveMessage"});
		}
		...
		if(jmiPro != null && jmiPro.attr["id"] != null) {
			...
			if(session != null) {
				try {
					...
					let e = haxe_Exception.caught(_g);
					haxe_Log.trace("JMI proceed failed",{ fileName : "borogove/Client.hx", lineNumber : 501, className : "borogove.Client", methodName : "processLiveMessage", customParams : [e]});
				}
			}
		}
		...
		if(stanza.attr["type"] != "error") {
			...
			if(userState != null) {
				...
				if(chat == null || !chat.getParticipantDetails(message.senderId).isSelf) {
					let _g = 0;
					let _g1 = this.chatStateHandlers;
					while(_g < _g1.length) {
						let handler = _g1[_g];
						++_g;
		...
		if(pubsubEvent != null && pubsubEvent.getFrom() != null) {
			let fromBare = borogove_JID.parse(pubsubEvent.getFrom()).asBare();
			let isOwnAccount = fromBare.asString() == this.accountId();
			...
			haxe_Log.trace("pubsubNode == " + pubsubNode,{ fileName : "borogove/Client.hx", lineNumber : 600, className : "borogove.Client", methodName : "processLiveMessage"});
		}
	}
	start() {
		let _gthis = this;
	...
	onConnected(data) {
		...
		this.rosterGet();
		haxe_Log.trace("SYNC: bookmarks",{ fileName : "borogove/Client.hx", lineNumber : 768, className : "borogove.Client", methodName : "onConnected"});
		this.bookmarksGet(function() {
			haxe_Log.trace("SYNC: MAM",{ fileName : "borogove/Client.hx", lineNumber : 770, className : "borogove.Client", methodName : "onConnected"});
			_gthis.sync(function(syncFinished) {
				if(!syncFinished) {
					haxe_Log.trace("SYNC: failed",{ fileName : "borogove/Client.hx", lineNumber : 773, className : "borogove.Client", methodName : "onConnected"});
					_gthis.inSync = false;
					_gthis.stream.disconnect();
					return;
				}
				haxe_Log.trace("SYNC: details",{ fileName : "borogove/Client.hx", lineNumber : 780, className : "borogove.Client", methodName : "onConnected"});
				_gthis.inSync = true;
				thenshim_Promise.then(_gthis.persistence.getChatsUnreadDetails(_gthis.accountId(),_gthis.chats),function(details) {
					...
					haxe_Log.trace("SYNC: done",{ fileName : "borogove/Client.hx", lineNumber : 806, className : "borogove.Client", methodName : "onConnected"});
				});
			});
		});
			...
			return thenshim_Promise._new(function(resolve,reject) {
				...
				tink_io_Source.chunked(new tink_io_js_BlobSource(source.name,source,0,chunkSize)).forEach(tink_streams_Handler.ofSafeSync(function(chunk) {
					...
					if(o._hx_index == 3) {
						...
						haxe_Log.trace("Error computing attachment hash",{ fileName : "borogove/Client.hx", lineNumber : 833, className : "borogove.Client", methodName : "prepareAttachment", customParams : [o]});
						reject(o);
					}
				});
			});
	...
	prepareAttachmentFor(source,services,hashes,callback) {
		if(services.length < 1) {
			haxe_Log.trace("No HTTP upload service found",{ fileName : "borogove/Client.hx", lineNumber : 842, className : "borogove.Client", methodName : "prepareAttachmentFor"});
			callback(null);
			return;
		}
		...
		httpUploadSlot.onFinished(function() {
			...
			if(slot == null) {
				...
				let options = slot.putHeaders.concat([new tink_http_HeaderField("Content-Length".toLowerCase(),tink_http_HeaderValue.ofInt(source.size))]);
				let options1 = null;
				let chunkSize = options1 == null || options1.chunkSize == null ? 16777216 : options1.chunkSize;
				tink_http_FetchResponse.all(tink_http_Fetch.fetch(url,{ method : "PUT", headers : options, body : tink_io_RealSourceTools.idealize(new tink_io_js_BlobSource(source.name,source,0,chunkSize),function(e) {
					haxe_Log.trace("WUT",{ fileName : "borogove/Client.hx", lineNumber : 852, className : "borogove.Client", methodName : "prepareAttachmentFor", customParams : [e]});
		...
		let checkAndAdd = function(jid,prepend) {
			...
			discoGet.onFinished(function() {
				...
				if(resultCaps == null) {
					...
					add(new borogove_AvailableChat(jid.asString(),displayName,note,resultCaps));
				}
				callback(q,results);
			});
			_gthis.sendQuery(discoGet);
		...
		while(_g < _g1.length) {
			...
			if(chat.isTrusted()) {
				...
				while(!_g_lastStep.done) {
					...
					let fullJid = new borogove_JID(bareJid.node,bareJid.domain,bareJid.isDomain() && resource == "" ? null : resource);
					let jigGet = new borogove_queries_JabberIqGatewayGet(fullJid.asString(),query);
					jigGet.onFinished(function() {
						if(jigGet.getResult() == null) {
							...
							case 1:
								let result = _g.v;
								checkAndAdd(borogove_JID.parse(result));
								break;
							}
						}
					});
		...
		window.crypto.subtle.exportKey("raw",vapid_key.publicKey).then(function(vapid_public_raw) {
			return reg.pushManager.subscribe({ userVisibleOnly : true, applicationServerKey : vapid_public_raw}).then(function(pushSubscription) {
				if(pushSubscription == null) {
					haxe_Log.trace("WebPush subscription failed",{ fileName : "borogove/Client.hx", lineNumber : 1039, className : "borogove.Client", methodName : "subscribePush"});
					return;
				}
	...
	enablePush(push_service,endpoint,p256dh,auth,grace,vapid_private_pkcs8,claims) {
		let tmp = claims;
		this.enabledPushData = { push_service : push_service, vapid_private_pkcs8 : vapid_private_pkcs8, endpoint : endpoint, p256dh : p256dh, auth : auth, grace : grace, claims : tmp != null ? tmp : []};
		let filters = [];
		let _g = 0;
		let _g1 = this.chats;
		while(_g < _g1.length) {
		...
		let value = tink_Url.parse(endpoint).hosts[0];
		_g2.h["aud"] = value;
		let claimMap = _g2;
		let _g3 = 0;
		let tmp1 = claims;
		let _g4 = (tmp1 != null ? tmp1 : []).length;
		while(_g3 < _g4) {
			let i = _g3++;
			if(i % 2 == 0) {
				let v = claims[i + 1];
				claimMap.h[claims[i]] = v;
			}
		}
		let tmp2 = this.jid.asBare().asString();
		let tmp3 = haxe_io_Bytes.ofData(p256dh);
		let tmp4 = haxe_io_Bytes.ofData(auth);
		let tmp5 = vapid_private_pkcs8 == null ? null : haxe_io_Bytes.ofData(vapid_private_pkcs8);
		this.sendQuery(new borogove_queries_Push2Enable(tmp2,push_service,endpoint,tmp3,tmp4,vapid_private_pkcs8 == null ? null : "ES256",tmp5,borogove_Map.fromMap(claimMap),grace,filters));
	}
	updatePushIfEnabled() {
	...
	addTlsCheckListener(handler) {
		this.stream.on("tls/check",function(data) {
	...
	addCallUpdateStatusListener(handler) {
		this.on("call/updateStatus",function(data) {
			handler(data.session);
			return borogove_EventResult.EventHandled;
		});
	}
	...
	doSync(callback,lastId) {
		...
		sync.onMessages(function(messageList) {
			...
			while(_g < _g1.length) {
				...
				case 3:
					let update = m.update;
					promises.push(thenshim_Promise.then(_gthis.persistence.storeReaction(_gthis.accountId(),update),function(_) {
						return null;
					}));
					break;
				default:
			...
			haxe_Log.trace("SYNC: MAM page wait for writes",{ fileName : "borogove/Client.hx", lineNumber : 1625, className : "borogove.Client", methodName : "doSync"});
			thenshim_Promise.then(thenshim_PromiseTools.all(promises),function(results) {
				...
				if(sync.hasMore()) {
					...
					while(!_g_lastStep1.done) {
						...
						let chatId = _g_key;
						let _ = _g_value;
						let chat = _gthis.getChat(chatId);
						if(chat == null) {
							_gthis.getDirectChat(chatId);
						}
					}
					if(callback != null) {
						callback(true);
				...
				haxe_Log.trace("SYNC: error",{ fileName : "borogove/Client.hx", lineNumber : 1654, className : "borogove.Client", methodName : "doSync", customParams : [e == null ? "null" : Std.string(e)]});
				callback(false);
			});
		});
		sync.onError(function(stanza) {
			if(lastId != null) {
				...
				haxe_Log.trace("SYNC: error",{ fileName : "borogove/Client.hx", lineNumber : 1663, className : "borogove.Client", methodName : "doSync", customParams : [stanza]});
				if(callback != null) {
					callback(false);
				}
			}
...
borogove_EventHandler.__name__ = "borogove.EventHandler";
Object.assign(borogove_EventHandler.prototype, {
	__class__: borogove_EventHandler
	,handlers: null
	,callback: null
	,onlyOnce: null
});
...
class borogove_GenericStream extends borogove_EventEmitter {
	...
	onStanza(stanza) {
		haxe_Log.trace("stanza received!",{ fileName : "borogove/GenericStream.hx", lineNumber : 40, className : "borogove.GenericStream", methodName : "onStanza"});
		let xmlns = stanza.attr["xmlns"];
		if(xmlns == "jabber:client") {
			let name = stanza.name;
			if(name == "iq") {
				let type = stanza.attr["type"];
				haxe_Log.trace("type: " + type,{ fileName : "borogove/GenericStream.hx", lineNumber : 46, className : "borogove.GenericStream", methodName : "onStanza"});
...
class borogove_Hash {
	...
	static fromUri(uri) {
		if(uri.startsWith("cid:") && uri.endsWith("@bob.xmpp.org") && uri.includes("+")) {
			...
			return borogove_Hash.fromHex(algo,parts[1]);
		}
...
class borogove_Identicon {
	static svg(source) {
		...
		uri += "<style>rect{fill:%23" + HxOverrides.substr(borogove_Color.forString(source),1,null) + ";}</style>";
		let i = 0;
		let value = hash >> i++;
		if(value % 2 == 0) {
...
var borogove_MessageStanza = $hxEnums["borogove.MessageStanza"] = { __ename__:true,__constructs__:null
	...
	,UndecryptableMessageStanza: ($_=function(decryptionFailure) { return {_hx_index:5,decryptionFailure:decryptionFailure,__enum__:"borogove.MessageStanza",toString:$estr}; },$_._hx_name="UndecryptableMessageStanza",$_.__params__ = ["decryptionFailure"],$_)
};
borogove_MessageStanza.__constructs__ = [borogove_MessageStanza.ErrorMessageStanza,borogove_MessageStanza.ChatMessageStanza,borogove_MessageStanza.ModerateMessageStanza,borogove_MessageStanza.ReactionUpdateStanza,borogove_MessageStanza.UnknownMessageStanza,borogove_MessageStanza.UndecryptableMessageStanza];
class borogove_Message {
	...
	static fromStanza(stanza,localJid,addContext,encryptionInfo) {
		...
		if(encryptionInfo != null && encryptionInfo.status == 1) {
			haxe_Log.trace("Message decryption failure: " + encryptionInfo.reasonText,{ fileName : "borogove/Message.hx", lineNumber : 63, className : "borogove.Message", methodName : "fromStanza"});
			return new borogove_Message(from,from,stanza.getChildText("thread"),borogove_MessageStanza.UndecryptableMessageStanza(encryptionInfo),encryptionInfo);
		}
		...
		if(msg.from != null && msg.from.equals(localJidBare)) {
			...
			if(carbon != null) {
				...
				if(fwd != null) {
					return borogove_Message.fromStanza(fwd.getFirstChild(),localJid,null,encryptionInfo);
				}
			}
		}
		...
		if(addresses != null) {
			...
			while(_g < _g1.length) {
				...
				if(address.attr["type"] == "noreply") {
					...
					haxe_Log.trace("No support for addressing to non-jid",{ fileName : "borogove/Message.hx", lineNumber : 146, className : "borogove.Message", methodName : "fromStanza", customParams : [address]});
		...
		while(x.hasNext()) {
			let x1 = x.next();
			_g2.push(borogove_JID.parse(x1));
		}
		msg.recipients = _g2;
		...
		if(tmp1) {
			haxe_Log.trace("Don't know what chat message without from in replyTo belongs in",{ fileName : "borogove/Message.hx", lineNumber : 174, className : "borogove.Message", methodName : "fromStanza", customParams : [stanza]});
			return new borogove_Message(msg.chatId(),msg.get_senderId(),msg.threadId,borogove_MessageStanza.UnknownMessageStanza(stanza),encryptionInfo);
		}
		if(addContext != null) {
		...
		if(reactionsEl != null) {
			...
			if(reactionId != null) {
				...
				while(_g < _g1) {
					let i = _g++;
					result[i] = new borogove_Reaction(msg.get_senderId(),timestamp,reactions[i],msg.localId);
				}
				return new borogove_Message(tmp,tmp1,msg1,borogove_MessageStanza.ReactionUpdateStanza(new borogove_ReactionUpdate(tmp3,tmp4,tmp5,tmp6,tmp7,tmp8,timestamp,result,0)),encryptionInfo);
			}
		}
		let _g4 = 0;
	...
	getReactions(existingReactions) {
		if(this.kind == 1) {
			...
			while(_g < _g1.length) {
				let r = _g1[_g];
				++_g;
				let custom = borogove_Util_downcast(r,borogove_CustomEmojiReaction);
				if(custom != null) {
					list.push(custom);
				}
			}
...
class borogove_Stanza {
	...
	serialize() {
		...
		let buffer_b = "";
		let len = serialized.length - 2;
		buffer_b += len == null ? HxOverrides.substr(serialized,0,null) : HxOverrides.substr(serialized,0,len);
		buffer_b += ">";
		let _g2 = 0;
		...
		while(_g2 < _g3.length) {
			...
			case 1:
				...
				x = borogove_Util_xmlEscape(c1);
				break;
			}
			buffer_b += Std.string(x);
		}
		buffer_b += "</";
		buffer_b += Std.string(this.name);
		buffer_b += ">";
		return buffer_b;
	}
	toString() {
	...
	textTag(tagName,textContent,attr) {
		let tmp = attr;
		this.last_added.addDirectChild(borogove_Node.Element(new borogove_Stanza(tagName,tmp != null ? tmp : { }).text(textContent)));
		return this;
	}
	up() {
	...
	clone() {
		...
		while(_g < _g1.length) {
			...
			case 0:
				let c = child.stanza;
				tmp = borogove_Node.Element(c.clone());
				break;
			case 1:
				let c1 = child.textNode;
				tmp = borogove_Node.CData(c1);
				break;
			}
	...
	getChildren() {
		...
		while(_g < _g1) {
			...
			case 1:
				...
				tmp = new borogove_TextNodeClass(text);
				break;
			}
			result[i] = tmp;
		}
	...
	find(path) {
		...
		do {
			...
			if(pos == len) {
				...
				return borogove_Node.Element(cursor.getChild(name1,xmlns));
			}
	...
	static parse(s) {
		return borogove_Stanza.fromXml(Xml.parse(s));
	}
	static fromXml(el) {
		if(el.nodeType == 6) {
			return borogove_Stanza.fromXml(el.firstElement());
		}
		let attrs = { };
		let a = el.attributes();
		while(a.hasNext()) {
		...
		let stanza = new borogove_Stanza(el.nodeName,attrs);
		...
		while(_g_current < _g_array.length) {
			...
			if(child.nodeType == 0) {
				stanza.addChild(borogove_Stanza.fromXml(child));
...
class borogove_XEP0393 {
	...
	static render(xhtml) {
		...
		while(_g < _g1.length) {
			let child = _g1[_g];
			++_g;
			s_b += Std.string(borogove_XEP0393.renderNode(child));
		}
		if(xhtml.name == "b" || xhtml.name == "strong") {
			s_b += "*";
		}
	...
	static renderNode(xhtml) {
		...
		case 0:
			let c = xhtml.stanza;
			return borogove_XEP0393.render(c);
		case 1:
...
class borogove_calls_IncomingProposedSession {
	...
	terminate() {
		haxe_Log.trace("Tried to terminate before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 114, className : "borogove.calls.IncomingProposedSession", methodName : "terminate", customParams : [this]});
	}
	contentAdd(_) {
		haxe_Log.trace("Got content-add before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 118, className : "borogove.calls.IncomingProposedSession", methodName : "contentAdd", customParams : [this]});
	}
	contentAccept(_) {
		haxe_Log.trace("Got content-accept before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 122, className : "borogove.calls.IncomingProposedSession", methodName : "contentAccept", customParams : [this]});
	}
	transportInfo(_) {
		haxe_Log.trace("Got transport-info before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 126, className : "borogove.calls.IncomingProposedSession", methodName : "transportInfo", customParams : [this]});
		return thenshim_Promise.resolve(null);
	}
	accept() {
		if(this.accepted) {
...
class borogove_calls_OutgoingProposedSession {
	...
	propose(audio,video) {
		...
		thenshim_Promise.then(this.client.storeMessages([msg]),function(stored) {
			...
			return _gthis.client.trigger("call/ringing",{ chatId : _gthis.get_chatId(), sid : _gthis.get_sid()});
		});
	}
	ring() {
		haxe_Log.trace("Tried to accept before initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 218, className : "borogove.calls.OutgoingProposedSession", methodName : "ring", customParams : [this]});
	}
	hangup() {
	...
	terminate() {
		haxe_Log.trace("Tried to terminate before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 241, className : "borogove.calls.OutgoingProposedSession", methodName : "terminate", customParams : [this]});
	}
	contentAdd(_) {
		haxe_Log.trace("Got content-add before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 245, className : "borogove.calls.OutgoingProposedSession", methodName : "contentAdd", customParams : [this]});
	}
	contentAccept(_) {
		haxe_Log.trace("Got content-accept before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 249, className : "borogove.calls.OutgoingProposedSession", methodName : "contentAccept", customParams : [this]});
	}
	transportInfo(_) {
		haxe_Log.trace("Got transport-info before session-initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 253, className : "borogove.calls.OutgoingProposedSession", methodName : "transportInfo", customParams : [this]});
		return thenshim_Promise.resolve(null);
	}
	accept() {
		haxe_Log.trace("Tried to accept before initiate: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 258, className : "borogove.calls.OutgoingProposedSession", methodName : "accept", customParams : [this]});
	}
	initiate(stanza) {
...
class borogove_calls_InitiatedSession {
	...
	retract() {
		haxe_Log.trace("Tried to retract session in wrong state: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 355, className : "borogove.calls.InitiatedSession", methodName : "retract", customParams : [this]});
	}
	accept() {
	...
	initiate(stanza) {
		haxe_Log.trace("Trying to initiate already initiated session: " + this.get_sid(),{ fileName : "borogove/calls/Session.hx", lineNumber : 378, className : "borogove.calls.InitiatedSession", methodName : "initiate"});
	...
	callStatus() {
		if(this.pc == null || this.pc.connectionState == "connecting") {
			return 3;
		...
		this.setupPeerConnection(function() {
			...
			if(_gthis.afterMedia == null) {
				thenshim_PromiseTools.catch_(_gthis.onPeerConnection(),function(e) {
					haxe_Log.trace("supplyMedia error",{ fileName : "borogove/calls/Session.hx", lineNumber : 557, className : "borogove.calls.InitiatedSession", methodName : "supplyMedia", customParams : [e == null ? "null" : Std.string(e)]});
					_gthis.pc.close();
				});
			} else {
				_gthis.afterMedia();
	...
	setupPeerConnection(callback) {
		...
		this.client.getIceServers(function(servers) {
			...
			_gthis.pc.addEventListener("track",function(event) {
				return _gthis.client.trigger("call/track",{ chatId : _gthis.get_chatId(), track : event.track, streams : event.streams});
			});
			_gthis.pc.addEventListener("negotiationneeded",function(event) {
				haxe_Log.trace("renegotiate",{ fileName : "borogove/calls/Session.hx", lineNumber : 576, className : "borogove.calls.InitiatedSession", methodName : "setupPeerConnection", customParams : [event]});
			});
...
class borogove_calls_SessionDescription {
	...
	addContent(newDescription) {
		...
		return new borogove_calls_SessionDescription(tmp,tmp1,tmp2,tmp3.concat(_g5),newDescription.identificationTags);
	}
	toSdp() {
	...
	static fromStanza(iq,initiator,existingDescription) {
		...
		return new borogove_calls_SessionDescription(0,"-",media,attributes,tags);
	}
}
...
class borogove_calls_Media {
	...
	toTransportElement(sessionAttributes) {
		...
		while(_g3 < _g4) {
			let i = _g3++;
			result[i] = borogove_calls_IceCandidate.parse(_this[i].value,_gthis.mid,ufragPwd.ufrag).toElement();
		}
	...
	static fromElement(content,initiator,hasGroup,existingDescription) {
		...
		while(_g < _g1.length) {
			...
			if(parameters.length > 0) {
				mediaAttributes.push(new borogove_calls_Attribute("fmtp",id + " " + parameters.join(";")));
			}
			let _g4 = 0;
			...
			while(_g6 < _g7.length) {
				let trrInt = _g7[_g6];
				++_g6;
				mediaAttributes.push(new borogove_calls_Attribute("rtcp-fb",id + " trr-int " + trrInt.attr["value"]));
			}
		}
		let _g2 = 0;
		...
		while(_g4 < _g5.length) {
			let trrInt = _g5[_g4];
			++_g4;
			mediaAttributes.push(new borogove_calls_Attribute("rtcp-fb","* trr-int " + trrInt.attr["value"]));
		}
		let _g6 = 0;
...
class borogove_calls_IceCandidate {
	...
	static fromStanza(iq) {
		...
		while(x.hasNext()) {
			...
			_g.push(borogove_calls_IceCandidate.fromTransport(transport,x1.attr["name"]));
		}
		let _g1 = [];
		let e = $getIterator(_g);
		while(e.hasNext()) {
	...
	storeChats(accountId,chats) {
		...
		this.storeChatTimer = haxe_Timer.delay(function() {
			let mapPresence = function(chat) {
				...
				while(!_g_lastStep.done) {
					...
					if(presence.caps != null) {
						let k = presence.caps.verRaw().hash;
						let v = presence.caps;
						caps.set(k,v);
						let tmp = resource;
			...
			while(x.hasNext()) {
				...
				let row6 = [accountId1,x2,row,x3,row1,x4,x5,row2,row3,x6,row4,row5,c.__name__.split(".").pop(),x1.notificationsFiltered(),x1.notifyMention(),x1.notifyReply()];
				_g.push(row6);
			}
			let _g1 = [];
			let e = $getIterator(_g);
	...
	getChats(accountId) {
		...
		return thenshim_Promise.then(thenshim_Promise.then(this.db.exec("SELECT chat_id, trusted, avatar_sha1, fn, ui_state, blocked, extensions, read_up_to_id, read_up_to_by, notifications_filtered, notify_mention, notify_reply, json(caps) AS caps, caps_ver, json(presence) AS presence, class FROM chats LEFT JOIN caps ON chats.caps_ver=caps.sha1 WHERE account_id=?",[accountId]),function(result) {
			...
			while(_g.current < _g.array.length) {
				...
				let v = new borogove_Caps(json.node,json.identities.map(function(i) {
					return new borogove_Identity(i.category,i.type,i.name);
			...
			while(true) {
				...
				while(_g < _g1.length) {
					...
					let v = new borogove_Presence(presence.caps == null ? null : capsMap.get(presence.caps),presence.mucUser == null || borogove_Config.constrainedMemoryMode ? null : borogove_Stanza.parse(presence.mucUser));
	...
	storeMessages(accountId,messages) {
		...
		return thenshim_Promise.then(thenshim_Promise.then(tmp,function(_) {
			...
			while(_g_current < _g_array.length) {
				...
				let tmp3 = correctable.localId;
				let tmp4 = tmp3 != null ? tmp3 : correctable.serverId;
				let correctable1 = correctable.syncPoint;
				let tmp5 = correctable.chatId();
				_g2.push([accountId1,tmp != null ? tmp : "",tmp1 != null ? tmp1 : "",tmp2 != null ? tmp2 : "",tmp4,correctable1,tmp5,correctable.senderId,message.timestamp,message.status,message.direction,message.type,message.asStanza().toString()]);
			}
			let _g3 = [];
				...
				if(row.versions != null) {
					...
					if(Reflect.fields(versions).length > 1) {
						...
						while(_g_index < _g_keys.length) {
							...
							let _g_value = _g_access[key];
							let _g_key = key;
							let versionId = _g_key;
							let version = _g_value;
							let versionM = borogove_ChatMessage.fromStanza(borogove_Stanza.parse(version),accountJid,function(toPushB,_) {
...
class borogove_queries_BoB extends borogove_queries_GenericQuery {
	...
	_hx_constructor(to,uri) {
		...
		this.queryStanza = new borogove_Stanza("iq",{ to : to, type : "get", id : this.queryId}).tag("data",{ xmlns : this.xmlns, cid : tmp}).up();
	}
	handleResponse(stanza) {
		this.responseStanza = stanza;
		this.finish();
...
class borogove_queries_DiscoInfoGet extends borogove_queries_GenericQuery {
	...
	getResult() {
		...
		if(this.result == null) {
			...
			this.result = new borogove_Caps(q1,tmp,result1);
		}
		return this.result;
	}
}
...
class borogove_streams_XmppJsStream extends borogove_GenericStream {
	...
	connectWithURI(uri) {
		haxe_Log.trace("Got connection URI: " + uri,{ fileName : "borogove/streams/XmppJsStream.hx", lineNumber : 165, className : "borogove.streams.XmppJsStream", methodName : "connectWithURI"});
		if(uri == null) {
		...
		let xmpp = new borogove_streams_XmppJsClient({ service : this.connectionURI, domain : this.jid.domain, resource : this.jid.resource, credentials : function(callback,mechanisms,fast) {
		...
		if(this.initialSM != null) {
			...
			if(parsedPending != null) {
				...
				while(_g < parsedPending.length) {
					...
					++_g;
					this.pending.push(borogove_streams_XmppJsLtx.parse(item));
				}
			}
		...
		xmpp.start().catch(function(err) {
			...
			if((xmppError != null ? xmppError.name : null) == "SASLError") {
				...
				haxe_Log.trace(err,{ fileName : "borogove/streams/XmppJsStream.hx", lineNumber : 298, className : "borogove.streams.XmppJsStream", methodName : "connectWithURI"});
			}
		});
	}
	connect(jid,sm) {
		this.state.event("connect-requested");
		this.jid = new borogove_streams_XmppJsJID(jid);
		this.initialSM = sm;
		borogove_streams_XmppJsStream.resolveConnectionURI(this.jid.domain,$bind(this,this.connectWithURI));
	}
	disconnect() {
		if(this.client == null) {
			return;
...
borogove_Version.HUMAN = "e87f864";
