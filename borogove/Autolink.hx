...
class Autolink {
	 ...
	 *  @hide
	 */
	static final IANA_TOP_LEVEL_DOMAINS =
			"(?:"
	...
	public static final GOOD_IRI_CHAR =
			"a-zA-Z0-9\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF";
	public static final IP_ADDRESS =
	 ...
	 */
	public static final IP6_ADDRESS =
					"\\[" +
	 ...
	 */
	private static final LABEL_CHAR = "a-zA-Z0-9" + UCS_CHAR;
	/**
	 * Valid characters for IRI TLD defined in RFC 3987.
	 */
	private static final TLD_CHAR = "a-zA-Z" + UCS_CHAR;
	/**
	 * RFC 1035 Section 2.3.4 limits the labels to a maximum 63 octets.
	 */
	private static final IRI_LABEL =
			"[" + LABEL_CHAR + "](?:[" + LABEL_CHAR + "\\-]{0,61}[" + LABEL_CHAR + "]){0,1}";
	/**
	 * RFC 3492 references RFC 1034 and limits Punycode algorithm output to 63 characters.
	 */
	private static final PUNYCODE_TLD = "xn\\-\\-[\\w\\-]{0,58}\\w";
	private static final TLD = "(" + PUNYCODE_TLD + "|" + "[" + TLD_CHAR + "]{2,63}" +")";
	private static final HOST_NAME = "(" + IRI_LABEL + "\\.)+" + TLD;
	/*public static final DOMAIN_NAME
		= new EReg("(" + HOST_NAME + "|" + IP6_ADDRESS + "|" + IP_ADDRESS +")", "u");*/
	private static final PROTOCOL = "(?:http|https|rtsp):\\/\\/";
	/* A word boundary or end of input.  This is to stop foo.sure from matching as foo.su */
	private static final WORD_BOUNDARY = "(?:\\b|$|^)";
	private static final USER_INFO = "(?:[a-zA-Z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)"
			+ "\\,\\;\\?\\&\\=]|(?:\\%[a-fA-F0-9]{2})){1,64}(?:\\:(?:[a-zA-Z0-9\\$\\-\\_"
			+ "\\.\\+\\!\\*\\'\\(\\)\\,\\;\\?\\&\\=]|(?:\\%[a-fA-F0-9]{2})){1,25})?\\@";
	private static final PORT_NUMBER = "\\:\\d{1,5}";
	private static final PATH_CHAR = "(?:(?:[" + LABEL_CHAR
			+ "\\;\\/\\?\\:\\@\\&\\=\\#\\~"
			+ "\\-\\.\\+\\!\\*\\'\\(\\)\\,\\_\\$])|(?:\\%[a-fA-F0-9]{2}))";
	private static final PATH_AND_QUERY = "\\/" + PATH_CHAR + "*";
	/**
	 *  Regular expression pattern to match most part of RFC 3987
	 *  Internationalized URLs, aka IRIs.
	 */
	/*public static final WEB_URL = new EReg("("
			+ "("
			+ "(?:" + PROTOCOL + "(?:" + USER_INFO + ")?" + ")?"
			+ "(?:" + DOMAIN_NAME + ")"
			+ "(?:" + PORT_NUMBER + ")?"
	 ...
	 */
	private static final STRICT_TLD = "(?:" +
			IANA_TOP_LEVEL_DOMAINS + "|" + PUNYCODE_TLD + ")";
	/**
	 * Regular expression that matches host names using {@link #STRICT_TLD}
	 */
	private static final STRICT_HOST_NAME = "(?:(?:" + IRI_LABEL + "\\.)+"
			+ STRICT_TLD + ")";
	/**
	 * Regular expression that matches domain names using either {@link #STRICT_HOST_NAME} or
	 * {@link #IP_ADDRESS}
	 */
	private static final STRICT_DOMAIN_NAME
		= "(?:" + STRICT_HOST_NAME + "|" + IP_ADDRESS + "|" + IP6_ADDRESS + ")";
	/**
	 * Regular expression that matches domain names without a TLD
	 */
	private static final RELAXED_DOMAIN_NAME =
	 ...
	 */
	private static final WEB_URL_WITHOUT_PROTOCOL = "("
			+ WORD_BOUNDARY
			+ "(?<!:\\/\\/)"
			+ "("
			+ "(?:" + STRICT_DOMAIN_NAME + ")"
	 ...
	 */
	private static final WEB_URL_WITH_PROTOCOL = "("
			+ WORD_BOUNDARY
			+ "(?:"
			+ "(?:" + PROTOCOL + "(?:" + USER_INFO + ")?" + ")"
			+ "(?:" + RELAXED_DOMAIN_NAME + ")?"
