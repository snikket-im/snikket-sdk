		...
		db = new SqliteDriver(dbfile, (exec) -> {
			exec(["PRAGMA user_version"]).then(iter -> {
				...
				return Promise.resolve(null).then(_ -> {
					...
					if (version < 16) {
						return exec(["CREATE TABLE omemo_prekeys (
							account_id TEXT NOT NULL,
					...
					if (version < 22) {
						return exec(["ALTER TABLE messages ADD COLUMN debug BLOB",
	...
	**/
	...
	private function hydrateMessages(
		...
		return { iterator: () -> rows }.map(row -> ChatMessage.fromStanza(Stanza.parse(row.stanza), accountJid, (builder, _) -> {
			...
			builder.debug = row.debug == null ? null : Json.parse(row.debug);
	...
	public function storeOmemoPreKey(accountId:String, keyId:Int, keyPair:PreKeyPair):Promise<PreKeyPair> {
		return db.exec(
			"INSERT OR REPLACE INTO omemo_prekeys VALUES (?,?,?,?)",
			[accountId, keyId, keyPair.privKey, keyPair.pubKey],
		).then(_ -> keyPair);
	}
	...
	public function getOmemoPreKey(accountId:String, keyId:Int): Promise<Null<PreKeyPair>> {
		return db.exec(
			"SELECT private_key, public_key FROM omemo_prekeys WHERE account_id=? AND key_id=? LIMIT 1",
			[accountId, keyId],
		).then(result -> {
			for (row in result) {
				return {
	...
	public function removeOmemoPreKey(accountId:String, keyId:Int):Promise<Bool> {
		return db.exec(
			"DELETE FROM omemo_prekeys WHERE account_id=? AND key_id=?",
			[accountId, keyId],
		).then(_ -> true);
	}
	...
	private function insertMessages(accountId:String, messages:Array<ChatMessage>) {
		final columns = [
			...
			insertCol("debug", (m) -> originalMessage(m).debug == null ? null : Json.stringify(originalMessage(m).debug), "jsonb(?)"),
