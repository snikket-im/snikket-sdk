...
class HaxeCBridge {
	...
	static public function expose(?namespace: String) {
		...
		for (field in forloop) {
			...
			case FVar(t, e):
					final convertedType = convertSecondaryType(t);
					if (convertedType.retainType == "Array") {
						final ptrT = TPath({name: "RawPointer", pack: ["cpp"], params: [TPType(convertedType.args[0])]});
						fields.insert(insertTo, {
							name: field.name + "__fromC",
							doc: field.doc,
							meta: [{name: "HaxeCBridge.wrapper", params: [], pos: field.pos}],
	...
	static function generateImplementation(ctx: CConverterContext, namespace: String) {
		...
		+ code('
			...
			HAXE_C_BRIDGE_LINKAGE
			void ${namespace}_stop(bool waitOnScheduledEvents) {
	...
	static function generateFunctionImplementation(namespace: String, d: CDeclaration) {
		...
		inline function castC2Cpp(expr: String, rootCType: CType) {
			...
			return switch rootCType {
				...
				case FunctionPointer(_): 'cpp::Function<${CPrinter.printType(rootCType, "")}>($expr)';
				case Ident(_), InlineStruct(_), Pointer(_): expr; // hxcpp auto casting works
			}
		}
		...
		if (externalThread) {
			...
			var argCTypes = haxeFunction.rootCTypes.args;
			return (
				code('
					HAXE_C_BRIDGE_LINKAGE
				')
...
class CPrinter {
	...
	public static function printType(cType: CType, argName: Null<String> = null): String {
		return switch cType {
			case Ident('HaxeString', modifiers):
				(hasModifiers(modifiers) ? (printModifiers(modifiers) + ' ') : '') + "const char *" + (argName == null ? '' : ' $argName');
			case Ident('HaxeObject', modifiers):
				(hasModifiers(modifiers) ? (printModifiers(modifiers) + ' ') : '') + "void *" + (argName == null ? '' : ' $argName');
			...
			case Enum(name):
				'enum $name' + (argName == null ? '' : ' $argName');
		}
	}
...
class CConverterContext {
	...
	**/
	...
	function getFunctionCType(args: Array<{name: String, opt: Bool, t: Type}>, ret: Type, pos: Position): CType {
		...
		if (false && generateTypedefForFunctions) {
			if (!declaredTypeIdentifiers.exists(ident)) {
	...
	function getHaxeObjectCType(t: Type): CType {
		...
		var functionIdent = '${declarationPrefix}_release';
		...
		/*if (!supportDeclaredTypeIdentifiers.exists(typeIdent)) {
			supportTypeDeclarations.push({
				...
				doc: code('
					...
					When passed from haxe to C, a reference to the object is retained to prevent garbage collection. You should call ${functionIdent} when finished with this handle in C to allow collection.')
			});
	...
	function getHaxeStringCType(t: Type): CType {
		getHaxeObjectCType(null); // Make sure free is set up
		// in the future we could specialize based on t (i.e. generating another typedef name like HaxeObject_SomeType)
		...
		/*if (!supportDeclaredTypeIdentifiers.exists(typeIdent)) {
			supportTypeDeclarations.push({
				...
				doc: code('
					...
					When passed from haxe to C, a reference to the object is retained to prevent garbage collection. You should call ${functionIdent} when finished with this handle to allow collection.')
			});
			supportDeclaredTypeIdentifiers.set(typeIdent, true);
		}*/

		if (false && !supportDeclaredFunctionIdentifiers.exists(functionIdent)) {
			supportFunctionDeclarations.push({
				doc: code('
...
abstract HaxeObject<T>(cpp.RawPointer<cpp.Void>) from cpp.RawPointer<cpp.Void> to cpp.RawPointer<cpp.Void> {
	...
	@:from
	public static inline function fromT<T>(x: T): HaxeObject<T> {
		return cast cpp.Pointer.addressOf(x);
	}
}
...
trace(e, haxe.CallStack.exceptionStack());
	...
	static public inline function retainHaxeArray<T>(haxeArray: Array<T>): HaxeArray<HaxeObject<T>> {
		// https://github.com/HaxeFoundation/hxcpp/issues/1092
